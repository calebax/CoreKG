...
// ImageQueryFile 图片搜索
func ImageQueryFile(ctx context.Context, forestID uint, url string) ([]*foresttype.KnownowForestFile, error) {
	foest_info, err := forest.GetForestByID(ctx, forestID)
	if err != nil {
		logs.ErrorContextf(ctx, "get forest info failed: %v", err)
		return nil, err
	}
	file_ids, err := ImageSearchFile(ctx, foest_info, url)
	if err != nil {
	...
	fileList, err := forest.GetForestFileByIDs(file_ids)
	if err != nil {
...
func DoImageParseRequest(ctx context.Context, url string) (string, error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[image_parse] [GetLLMConfig] error: %v", err)
		return "", err
	}
