...
const (
	initResourceStorage = "storage"
)

func resourceInit(opts map[string]struct{}) error {
	ctx := lifecycle.Std().Context()
	loadConfig(ctx, "../conf/test.yaml")
	for k := range opts {
		switch k {
		case initResourceStorage:
...
func TestDoAgentReq(t *testing.T) {
	client := agentclient.NewChatClient(nil, "http://localhost:8080/v3/chat.Agent/chat/completions", "CHANGE_ME_API_KEY")
	req := &agentclient.ChatRequestBody{
		Model: "5xpxiw4",
		Messages: []agentclient.Message{
			{
				Role:    agentclient.RoleUser,
				Content: agentclient.MessageContent{Text: "知识库里面有什么"},
			},
		},
		Stream: false,
	}
...
func TestDoAgentReq2(t *testing.T) {
	client := agentclient.NewChatClient(nil, "http://localhost:8080/v3/chat.Agent/chat/completions", "CHANGE_ME_API_KEY")
	req := &agentclient.ChatRequestBody{
		Model: "akAGVou",
		Messages: []agentclient.Message{
			{
				Role: agentclient.RoleSystem,
				Content: agentclient.MessageContent{
					Text: "你是一个专业的分析师",
				},
			},
			{
				Role: agentclient.RoleAssistant,
				Content: agentclient.MessageContent{
					Text: "你好，我是你的专业分析师，我可以帮你分析今天的计划",
				},
			},
			{
				Role: agentclient.RoleUser,
...
func TestAliyunDeepseekR1Chat(t *testing.T) {
	// 初始化 MessageContent
	client := agentclient.NewChatClient(nil, "http://localhost:8080/v3/chat.Agent/chat/completions", "CHANGE_ME_API_KEY")
	req := &agentclient.ChatRequestBody{
		Model: "o5kiIlN",
		Messages: []agentclient.Message{
			{
				Role:    "user",
				Content: agentclient.MessageContent{Text: "你好,知识库里有什么"},
			},
		},
	}
	resp, err := client.SendChat(context.Background(), req)
...
func TestChatStreamWithCallback(t *testing.T) {
	resp, err := agentclient.NewChatClient(nil,
		"https://api.example.com/v3/chat.Agent/chat/completions",
		"CHANGE_ME_API_KEY").
		SendChat(context.Background(), &agentclient.ChatRequestBody{
			Model: "JAXwWUd",
			ChatOptions: agentclient.ChatOptions{
				Input: []chattype.Input{
					{Name: "input1", Value: "文件内容仅包含一张图片链接无其他信息[]!(https://app.corekg.com/agents/182/edit)"},
				},
			},
		})
	if err != nil {
		t.Fatalf("非流式请求失败: %v", err)
	}
	fmt.Println(resp)
}
