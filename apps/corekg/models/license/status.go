...
const (
	StatusValid            ValidationStatus = 0 // 0: 许可证有效
	StatusInvalidFormat    ValidationStatus = 1 // 1: 格式错误
	StatusInvalidSignature ValidationStatus = 2 // 2: 签名无效
	StatusUIDMismatch      ValidationStatus = 3 // 3: UID 不匹配
	StatusExpired          ValidationStatus = 4 // 4: 已过期
	StatusTampered         ValidationStatus = 5 // 5: 哈希链被篡改
	StatusEnvError         ValidationStatus = 6 // 6: 环境错误（无法获取UID、License文件等）
	StatusInternalError    ValidationStatus = 7 // 7: 内部错误（如数据库、配置问题）
)
