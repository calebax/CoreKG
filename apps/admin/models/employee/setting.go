...
func QuerySettingList(ctx context.Context, opt apiobj.PageQuery, resp *QuerySettingListResponse) (err error) {
	query := dbutil.Core().WithContext(ctx).Table(settings.TableNameSettings).Where("deleted_at is null")
	...
	for _, filter := range opt.Filters {
		...
		case "group":
			query = query.Where("`group` LIKE ?", "%"+filter.Value[0]+"%")
		case "key":
			query = query.Where("`key` LIKE ?", "%"+filter.Value[0]+"%")
		default:
