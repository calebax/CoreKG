...
// 	Request admintype.CreatePositionOption
// }
...
func (in *SendCodeForgotPasswordRequest) Validity(out *apiobj.BaseResponse) {
	...
	if in.Request.By != "email" && in.Request.By != "" {
		...
		out.Message = fmt.Sprintf("暂不支持的方式 %s", in.Request.By)
		return
	}
	if err := validate.IsEmail(in.Request.Value); err != nil {
		out.Code = errcode.ErrCode_BadRequest
		out.Message = fmt.Sprintf("错误的邮箱格式, %s", err.Error())
		return
	}
}
...
func (in *ListEmployeeRequest) Validity(out *apiobj.BaseResponse) {
	if in.Request.Offset < 0 || in.Request.Limit < 0 {
		out.Code = errcode.ErrCode_BadRequest
		out.Message = "offset和limit必须大于0"
		return
	}
	for _, v := range in.Request.OrderBy {
		switch v {
		case "username", "email", "mobile", "created_at", "updated_at",
			"username desc", "email desc", "mobile desc", "created_at desc", "updated_at desc":
		default:
			out.Code = errcode.ErrCode_BadRequest
			out.Message = "orderBy不能为空"
			return
		}
	}
	for _, v := range in.Request.Filters {
		switch v.Field {
		case "auto", "username", "real_name", "email", "mobile", "status":
			if len(v.Value) != 1 {
				out.Code = errcode.ErrCode_BadRequest
				out.Message = "查询条件中的字段只能有一个值"
				return
			}
			if v.Value[0] == "" {
				out.Code = errcode.ErrCode_BadRequest
				out.Message = "查询条件中的值不能为空"
				return
			}

		default:
			out.Code = errcode.ErrCode_BadRequest
			out.Message = "查询条件中的字段不存在, " + v.Field
			return
		}
	}
}
...
// 			out.Code = errcode.ErrCode_BadRequest
// 			out.Message = fmt.Sprintf("错误的手机号格式, %s", err.Error())
// 			return
// 		}
// 	}
...
func (opt *EmployeeValidityOption) Validity(resp *apiobj.BaseResponse) {
	...
	if err := validate.IsEmail(opt.Email); err != nil {
		resp.Code = errcode.ErrCode_BadRequest
		resp.Message = fmt.Sprintf("错误的邮箱格式, %s", err.Error())
		return
	}
	if opt.Phone != "" {
		if err := validate.IsPhone(opt.Phone); err != nil {
			resp.Code = errcode.ErrCode_BadRequest
			resp.Message = fmt.Sprintf("错误的手机号格式, %s", err.Error())
			return
		}
	}
	if len(opt.PositionIDs) < 1 {
		resp.Code = errcode.ErrCode_BadRequest
		resp.Message = fmt.Sprintf("职位不能为空")
		return
	}
}
...
func (req *CreateEmployeeRequest) Validity(resp *apiobj.BaseResponse) {
	...
	in.Validity(resp)
	if len(in.Password) > 0 && len(in.Password) < 6 {
		resp.Code = errcode.ErrCode_BadRequest
		resp.Message = "密码长度不能小于6位"
		return
	}
	return
}
...
func (in *UpdateEmployeeRequest) Validity(out *apiobj.BaseResponse) {
	...
	if in.Request.Username != "" {
		if !isValidUsername(in.Request.Username) {
			out.Code = errcode.ErrCode_BadRequest
			out.Message = "用户名必须为英文或数字"
			return
		}
	}
}

// UpdateEmployeePasswordRequest 更新用户请求
...
// Validity 验证有效性
func (in *UpdateEmployeePasswordRequest) Validity(out *apiobj.BaseResponse) {
	if in.Request.ID == 0 {
		out.Code = errcode.ErrCode_BadRequest
		out.Message = "请选择员工"
		return
...
func (req *ModifyPositionInfoRequest) Validity(resp *apiobj.BaseResponse) {
	if req.Request.ID == 0 {
		resp.Code = errcode.ErrCode_BadRequest
		resp.Message = fmt.Sprintf("ID不能为空")
		return
	}
	if req.Request.Name == "" {
		resp.Code = errcode.ErrCode_BadRequest
...
func (req *ListPrivilegeRequest) Validity(resp *apiobj.BaseResponse) {
	...
	for _, v := range req.Request.OrderBy {
		switch v {
		case "name", "created_at", "updated_at",
			"name desc", "created_at desc", "updated_at desc", "action_path", "api", "action":
		default:
			resp.Code = errcode.ErrCode_BadRequest
			resp.Message = "orderBy字段不支持"
			return
	...
	for _, v := range req.Request.Filters {
		switch v.Field {
		case "name", "type", "action_path", "api", "action":
			if len(v.Value) != 1 {
				resp.Code = errcode.ErrCode_BadRequest
				resp.Message = "查询条件中的字段只能有一个值"
				return
...
func (req *BindEmployeeWechatRequest) Validity(resp *employee.BindEmployeeWechatResponse) {
	if req.Request.Code == "" || req.Request.DomainName == "" {
		resp.Code = errcode.ErrCode_BadRequest
		resp.Message = fmt.Sprintf("参数错误")
		return
	}
}
