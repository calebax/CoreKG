...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/infra/embedding/impl"
...
func SaveBasicConfiguration(ctx context.Context, c *app.RequestContext) {
	...
	logs.InfoContextf(ctx,"server host is valid %s", req.Configuration.ServerHost)
...
func UpdateKnowledgeConfig(ctx context.Context, c *app.RequestContext) {
	...
	logs.DebugContextf(ctx, "embed test string result: %d, expect %d",
...
func CreateModel(ctx context.Context, c *app.RequestContext) {
	...
	logs.DebugContextf(ctx, "create model req: %s, conn: %s", conv.DebugJsonToStr(req), conv.DebugJsonToStr(req.Connection.BaseConnInfo))
	...
	logs.DebugContextf(ctx, "chatModel.Generate resp : %s", conv.DebugJsonToStr(respMsgs))
