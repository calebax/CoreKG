...
var needAuthPath = map[string]bool{
	...
	"/v1/workflow/ygrun":                  false,
	"/v1/workflow/stream_run":             false,
...
func OpenapiAuthMW() app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "OpenAuthApplication.CheckPermission failed, err=%v", err)
			httputil.InternalError(ctx, c,
		...
		logs.InfoContextf(ctx, "OpenapiAuthMW: apiKeyInfo=%v", conv.DebugJsonToStr(apiKeyInfo))
		...
		if err != nil {
			logs.ErrorContextf(ctx, "OpenAuthApplication.UpdateLastUsedAt failed, err=%v", err)
		}
		c.Next(ctx)
	}
}
