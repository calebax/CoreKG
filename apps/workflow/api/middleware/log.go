...
func AccessLogMW() app.HandlerFunc {
	return func(c context.Context, ctx *app.RequestContext) {
		...
		case status >= http.StatusInternalServerError:
			logs.ErrorContextf(c, "%s", baseLog)
		case status >= http.StatusBadRequest:
			logs.WarnContextf(c, "%s", baseLog)
		default:
			...
			if requestAuthType != int32(RequestAuthTypeStaticFile) && filepath.Ext(path) == "" {
				logs.InfoContextf(c, "%s ", baseLog)
				logs.DebugContextf(c, "query : %s \nreq : %s \nresp: %s",
...
func SetLogIDMW() app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		ctx = logs.WithContextFields(ctx, "reqid", reqID)

		c.Header("X-Log-ID", reqID)
		c.Next(ctx)
	}
}
