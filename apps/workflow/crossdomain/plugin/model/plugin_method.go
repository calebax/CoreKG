...
func (p PluginInfo) GetToolExample(ctx context.Context, toolName string) *ToolExample {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "marshal request example failed, err=%v", err)
		return nil
	}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "marshal response example failed, err=%v", err)
		return nil
	}
...
func (p PluginInfo) GetSourceType() string {
	if p.SourceType == nil {
		return ""
	}
	return *p.SourceType
