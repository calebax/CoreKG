...
import (
	...
	plugin "github.com/insmtx/corekg/apps/workflow/crossdomain/plugin/model"
	...
	commonConsts "github.com/insmtx/corekg/apps/workflow/types/consts"
...
func (a *appServiceImpl) PublishAPP(ctx context.Context, req *PublishAPPRequest) (resp *PublishAPPResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "publish by connectors failed, recordID=%d, err=%v", recordID, err)
	}
...
func (a *appServiceImpl) publishByConnectors(ctx context.Context, recordID int64, req *PublishAPPRequest) (success bool, err error) {
	defer func() {
		if err != nil {
			...
			if updateErr != nil {
				logs.ErrorContextf(ctx, "UpdateAPPPublishStatus failed, recordID=%d, err=%v", recordID, updateErr)
			}
		}
	}()
	...
	if len(failedResources) > 0 {
		logs.WarnContextf(ctx, "packResources failed, recordID=%d, len=%d", recordID, len(failedResources))

		processErr := a.packResourcesFailedPostProcess(ctx, recordID, failedResources)
		if processErr != nil {
			logs.ErrorContextf(ctx, "packResourcesFailedPostProcess failed, recordID=%d, err=%v", recordID, processErr)
		}
	...
	for cid := range req.ConnectorPublishConfigs {
		...
		case commonConsts.APIConnectorID, commonConsts.WebSDKConnectorID:
			...
			logs.ErrorContextf(ctx, "failed to update connector '%d' publish status to success, err=%v", cid, updateSuccessErr)
			...
			if updateFailedErr != nil {
				logs.WarnContextf(ctx, "failed to update connector '%d' publish status to failed, err=%v", cid, updateFailedErr)
			}
