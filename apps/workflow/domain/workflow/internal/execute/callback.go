...
func (w *WorkflowHandler) initWorkflowCtx(ctx context.Context) (context.Context, bool) {
	...
	if w.subWorkflowBasic == nil {
		if w.resumeEvent != nil {
			...
			if err != nil {
				logs.ErrorContextf(ctx,"failed to restore root execute context: %v", err)
				return ctx, false
			}
		} else {
			newCtx, err = PrepareRootExeCtx(ctx, w)
			if err != nil {
				logs.ErrorContextf(ctx,"failed to prepare root exe context: %v", err)
				return ctx, false
			}
		}
	} else {
		...
		if resume {
			newCtx, err = restoreWorkflowCtx(ctx, w)
			if err != nil {
				logs.ErrorContextf(ctx,"failed to restore sub execute context: %v", err)
				return ctx, false
			}
		} else {
...
func (w *WorkflowHandler) OnStart(ctx context.Context, info *callbacks.RunInfo, input callbacks.CallbackInput) context.Context {
	...
	if w.subWorkflowBasic == nil {
		...
		if err != nil {
			logs.ErrorContextf(ctx,"failed to get workflow cancel flag: %v", err)
		}
...
func (w *WorkflowHandler) OnError(ctx context.Context, info *callbacks.RunInfo, err error) context.Context {
	...
	if ok {
		...
		if err != nil {
			logs.ErrorContextf(ctx,"failed to extract interrupt events: %v", err)
			return ctx
		}

		for _, interruptEvent := range interruptEvents {
			logs.InfoContextf(ctx, "emit interrupt event id= %d, eventType= %d, nodeID= %s", interruptEvent.ID,
	...
	logs.ErrorContextf(ctx, "workflow failed: %v", err)
...
func (w *WorkflowHandler) OnStartWithStreamInput(ctx context.Context, info *callbacks.RunInfo,
	...
	for {
		...
		if e != nil {
			...
			logs.ErrorContextf(ctx,"failed to receive stream input: %v", e)
			return newCtx
		}
		fullInput, e = nodes.ConcatTwoMaps(fullInput, chunk.(map[string]any))
		if e != nil {
			logs.ErrorContextf(ctx,"failed to concat two maps: %v", e)
			return newCtx
		}
	}
	c := GetExeCtx(newCtx)
	...
	safego.Go(ctx, func() {
		...
		for {
			...
			if e != nil {
				...
				logs.ErrorContextf(ctx,"workflow OnEndWithStreamOutput failed to receive stream output: %v", e)
...
func (n *NodeHandler) initNodeCtx(ctx context.Context, typ entity.NodeType) (context.Context, bool) {
	...
	if resume {
		...
		if err != nil {
			logs.ErrorContextf(ctx,"failed to restore node execute context: %v", err)
			return ctx, resume
		}
		...
		logs.InfoContextf(ctx, "[restoreNodeCtx] restored nodeKey= %s, root.resumeEventID= %d", n.nodeKey, resumeEventID)
	} else {
		...
		if restored {
			logs.InfoContextf(ctx, "[tryRestoreNodeCtx] restored, nodeKey= %s", n.nodeKey)
			newCtx = restoredCtx
			return newCtx, true
		}
		...
		if err != nil {
			logs.ErrorContextf(ctx,"failed to prepare node execute context: %v", err)
			return ctx, resume
		}
	}
...
func (n *NodeHandler) OnError(ctx context.Context, info *callbacks.RunInfo, err error) context.Context {
	...
	if _, ok := compose.IsInterruptRerunError(err); ok { // current node interrupts
		if err := compose.ProcessState[ExeContextStore](ctx, func(ctx context.Context, state ExeContextStore) error {
			...
			logs.InfoContextf(ctx, "[SetNodeCtx] nodeKey= %s", n.nodeKey)
			return state.SetNodeCtx(n.nodeKey, c)
		}); err != nil {
			logs.ErrorContextf(ctx,"failed to process state: %v", err)
		}
	...
	safego.Go(ctx, func() {
		...
		for {
			...
			if e != nil {
				...
				logs.ErrorContextf(ctx,"node OnStartWithStreamInput failed to receive stream output: %v", e)
...
func (t *ToolHandler) OnStart(ctx context.Context, info *callbacks.RunInfo,
	...
	if input.ArgumentsInJSON != "" {
		if err := sonic.UnmarshalString(input.ArgumentsInJSON, &args); err != nil {
			logs.ErrorContextf(ctx,"failed to unmarshal arguments: %v", err)
			return ctx
		}
	}
	...
	safego.Go(ctx, func() {
		...
		for {
			...
			if e != nil {
				...
				logs.ErrorContextf(ctx,"tool OnEndWithStreamOutput failed to receive stream output: %v", e)
