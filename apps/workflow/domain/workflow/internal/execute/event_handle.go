...
func handleEvent(ctx context.Context, event *Event, repo workflow.Repository,
	...
	case WorkflowStart:
		...
		if parentNodeID != nil { // root workflow execution has already been created
			var logID string
			logID, _ = ctx.Value("reqid").(string)
	...
	case WorkflowFailed:
		...
		logs.ErrorContextf(ctx, "workflow execution failed: %v", event.Err)
		...
		if cause := errors.Unwrap(event.Err); cause != nil {
			logs.ErrorContextf(ctx, "workflow %d for exeID %d returns err: %v, cause: %v",
				wfID, exeID, event.Err, cause)
		} else {
			logs.ErrorContextf(ctx, "workflow %d for exeID %d returns err: %v",
	...
	case NodeEnd, NodeEndStreaming:
		...
		if event.Err != nil {
			...
			if cause := errors.Unwrap(event.Err); cause != nil {
				logs.WarnContextf(ctx, "node %s for exeID %d end with warning: %v, cause: %v",
					event.NodeKey, event.NodeExecuteID, event.Err, cause)
			} else {
				logs.WarnContextf(ctx, "node %s for exeID %d end with warning: %v",
	...
	case NodeError:
		...
		if cause := errors.Unwrap(event.Err); cause != nil {
			logs.ErrorContextf(ctx, "node %s for exeID %d returns err: %v, cause: %v",
				event.NodeKey, event.NodeExecuteID, event.Err, cause)
		} else {
			logs.ErrorContextf(ctx, "node %s for exeID %d returns err: %v",
				event.NodeKey, event.NodeExecuteID, event.Err)
		}
...
func HandleExecuteEvent(ctx context.Context,
	...
	handler := func(event *Event) *Event {
		...
		logs.InfoContextf(ctx, "receiving event type= %v, workflowID= %v, nodeType= %v, nodeKey = %s",
			event.Type, event.RootWorkflowBasic.ID, nodeType, nodeKey)

		signal, err := handleEvent(ctx, event, repo, sw)
		if err != nil {
			logs.ErrorContextf(ctx, "failed to handle event: %v", err)
		}
		...
		case workflowSuccess: // workflow success, wait for exit node to be done
			...
			if lastNodeIsDone || exeCfg.Mode == workflowModel.ExecuteModeNodeDebug {
				if err = setRootWorkflowSuccess(ctx, wfSuccessEvent, repo, sw); err != nil {
					logs.ErrorContextf(ctx, "failed to set root workflow success for workflow %d: %v",
	...
	if exeCfg.Cancellable {
		...
		defer func() {
			logs.InfoContextf(ctx, "[handleExecuteEvent] cancellable finish, returned event type: %v, workflow id: %d",
				event.Type, event.Context.RootWorkflowBasic.ID)
			waitUntilToolFinish(ctx)
			logs.InfoContextf(ctx, "[handleExecuteEvent] cancellable wait until tool finished done, workflow id: %d",
		...
		for {
			...
			case <-cancelTicker.C:
				...
				if err != nil {
					logs.ErrorContextf(ctx, "failed to check cancellation status for workflow %d: %v", wfExeID, err)
					continue
				}

				if isCancelled {
					cancelled = true
					logs.InfoContextf(ctx, "workflow %d cancellation detected", wfExeID)
					cancelFn()
				}
		...
		defer func() {
			logs.InfoContextf(ctx, "[handleExecuteEvent] finish, returned event type: %v, workflow id: %d",
				event.Type, event.Context.RootWorkflowBasic.ID)
			waitUntilToolFinish(ctx)
			logs.InfoContextf(ctx, "[handleExecuteEvent] wait until tool finished done, workflow id: %d",
				event.Context.RootWorkflowBasic.ID)
			if timeoutFn != nil {
				timeoutFn()
			}
...
func cacheToolStreamingResponse(ctx context.Context, event *Event) {
	...
	logs.InfoContextf(ctx, "receive tool response: %s, callID: %s",
	...
	for _, m := range c {
		for _, info := range m {
			if info.toolFinishChan != nil {
				<-info.toolFinishChan
				logs.InfoContextf(ctx, "tool finished, callID: %s, pluginID: %v", info.output.CallID,
					info.input.PluginID)
			}
		}
	}
