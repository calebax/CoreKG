...
func isDebugExecute(ctx context.Context) (bool, error) {
	execCtx := execute.GetExeCtx(ctx)
	if execCtx == nil {
		return false, fmt.Errorf("unable to get exe context")
	}
	return execCtx.RootCtx.ExeCfg.Mode == workflowModel.ExecuteModeDebug || execCtx.RootCtx.ExeCfg.Mode == workflowModel.ExecuteModeNodeDebug, nil
}

func getExecUserID(ctx context.Context) (string, error) {
	execCtx := execute.GetExeCtx(ctx)
	if execCtx == nil {
		return "", fmt.Errorf("unable to get exe context")
	}
	if execCtx.RootCtx.ExeCfg.ConnectorUID != "" {
		return execCtx.RootCtx.ExeCfg.ConnectorUID, nil
	}
...
func getConnectorID(ctx context.Context) (int64, error) {
	execCtx := execute.GetExeCtx(ctx)
	if execCtx == nil {
		return 0, fmt.Errorf("unable to get exe context")
	}
	return execCtx.RootCtx.ExeCfg.ConnectorID, nil
}
