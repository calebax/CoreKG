...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/bizpkg/config/modelmgr"
	...
	wrapPlugin "github.com/insmtx/corekg/apps/workflow/domain/workflow/plugin"
...
func jsonParse(ctx context.Context, data string, schema_ map[string]*vo.TypeInfo) (map[string]any, error) {
	...
	if err != nil {
		...
		if c != nil {
			logs.ErrorContextf(ctx, "failed to parse json: %v, data: %s", err, data)
	...
	if resumingEvent != nil {
		...
		chatModelHandler := callbacks2.NewHandlerHelper().ChatModel(&callbacks2.ModelCallbackHandler{
			OnStart: func(ctx context.Context, runInfo *callbacks.RunInfo, input *model.CallbackInput) context.Context {
				...
				if e != nil {
					logs.ErrorContextf(ctx, "failed to pop first interrupt event on react agent chatmodel start: %v", err)
					return ctx
				}

				if !deleted {
					logs.ErrorContextf(ctx, "failed to pop first interrupt event on react agent chatmodel start: not deleted")
					return ctx
				}

				if ie.ID != resumingEvent.ID {
					logs.ErrorContextf(ctx, "failed to pop first interrupt event on react agent chatmodel start, "+
...
func (l *LLM) ToCallbackInput(ctx context.Context, input map[string]any) (
	*nodes.StructuredCallbackInput, error) {
	...
	for _, msg := range messages[startIdx:] {
		...
		if err != nil {
			logs.WarnContextf(ctx, "failed to convert message to string: %v", err)
			continue
		}
