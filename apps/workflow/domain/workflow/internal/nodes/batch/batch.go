...
func (b *Batch) Invoke(ctx context.Context, in map[string]any, opts ...nodes.NodeOption) (
	...
	ithTask := func(i int) {
		...
		mu.Unlock()
		if subCheckpointID != "" {
			logs.InfoContextf(ctx, "[testInterrupt] prepare %d th run for batch node %s, subCheckPointID %s",
	...
	if existingCState != nil && len(existingCState.Index2InterruptInfo) > 0 {
		logs.InfoContextf(ctx, "no interrupt thrown this round, but has historical interrupt events yet to be resumed, "+
