...
func validatePythonImports(code string) error {
	...
	pythonThirdPartyWhitelist := make(map[string]bool)
	if repo := wf.GetRepository(); repo != nil {
		if cfg := repo.GetNodeOfCodeConfig(); cfg != nil {
			pythonThirdPartyWhitelist = slices.ToMap(cfg.GetSupportThirdPartModules(), func(e string) (string, bool) {
				return e, true
			})
		}
	}
...
func (c *Runner) Invoke(ctx context.Context, input map[string]any) (ret map[string]any, err error) {
	...
	if c.runner == nil {
		return nil, vo.WrapError(errno.ErrCodeExecuteFail, fmt.Errorf("code runner not initialized"))
	}
