...
import (
	...
	workflow3 "github.com/insmtx/corekg/apps/workflow/api/model/workflow"
	...
	"github.com/insmtx/corekg/apps/workflow/domain/workflow/internal/repo/dal/model"
	"github.com/insmtx/corekg/apps/workflow/domain/workflow/internal/repo/dal/query"
...
func (r *RepositoryImpl) CreateVersion(ctx context.Context, id int64, info *vo.VersionInfo, newRefs map[entity.WorkflowReferenceKey]struct{}) (err error) {
	...
	if result.RowsAffected == 0 {
		logs.WarnContextf(ctx, "update workflow draft when publish failed: no rows affected. WorkflowID: %d", id)
	}
	...
	if err != nil {
		logs.WarnContextf(ctx, "update workflow meta when publish failed: %v", err)
	}
...
func (r *RepositoryImpl) convertMeta(ctx context.Context, meta *model.WorkflowMeta) (*vo.Meta, error) {
	...
	if err != nil {
		logs.WarnContextf(ctx,"failed to get url for workflow meta %v", err)
	}
...
func (r *RepositoryImpl) CreateSnapshotIfNeeded(ctx context.Context, id int64, commitID string) error {
	...
	if err != nil {
		if !errors.Is(err, gorm.ErrRecordNotFound) {
			logs.ErrorContextf(ctx, "query workflow snapshot failed err=%v", err)
		}
...
func (r *RepositoryImpl) CopyWorkflow(ctx context.Context, workflowID int64, policy vo.CopyWorkflowPolicy) (
	...
	if policy.ShouldModifyWorkflowName {
		...
		if err != nil {
			logs.WarnContextf(ctx,"failed to set the rediskey %v expiration time, err=%v", copiedWorkflowRedisKey, err)
		}
