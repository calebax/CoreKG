...
import (
	...
	wf "github.com/insmtx/corekg/apps/workflow/domain/workflow"
...
func (r *WorkflowRunner) Prepare(ctx context.Context) (
	...
	if interruptEvent != nil {
		...
		logs.InfoContextf(ctx, "resuming with eventID: %d, executeID: %d, nodeKey: %s", interruptEvent.ID,
	...
	go func() {
		defer func() {
			if panicErr := recover(); panicErr != nil {
				logs.ErrorContextf(ctx, "panic when handling execute event: %v", safego.NewPanicErr(panicErr, debug.Stack()))
			}
		}()
		defer func() {
			if container != nil {
