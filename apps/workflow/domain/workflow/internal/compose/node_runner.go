...
import (
	...
	openauthentity "github.com/insmtx/corekg/apps/workflow/domain/openauth/openapiauth/entity"
	...
	schema2 "github.com/insmtx/corekg/apps/workflow/domain/workflow/internal/schema"
	"github.com/insmtx/corekg/apps/workflow/pkg/ctxcache"
	"github.com/insmtx/corekg/apps/workflow/pkg/errorx"
	exec "github.com/insmtx/corekg/apps/workflow/pkg/execute"
...
// initCtxCachePreservingAuth resets the ctx cache while keeping request-level auth metadata.
func initCtxCachePreservingAuth(ctx context.Context) context.Context {
	preserved := preserveCtxCacheAuth(ctx)
...
func preserveCtxCacheAuth(ctx context.Context) map[any]any {
	preserved := make(map[any]any)

	if session, ok := ctxcache.Get[*userentity.Session](ctx, consts.SessionDataKeyInCtx); ok && session != nil {
		preserved[consts.SessionDataKeyInCtx] = session
	}

	if apiKey, ok := ctxcache.Get[*openauthentity.ApiKey](ctx, consts.OpenapiAuthKeyInCtx); ok && apiKey != nil {
		preserved[consts.OpenapiAuthKeyInCtx] = apiKey
	}

	if host, ok := ctxcache.Get[string](ctx, consts.HostKeyInCtx); ok && host != "" {
		preserved[consts.HostKeyInCtx] = host
	}

	if scheme, ok := ctxcache.Get[string](ctx, consts.RequestSchemeKeyInCtx); ok && scheme != "" {
		preserved[consts.RequestSchemeKeyInCtx] = scheme
	}
	...
	if meta.UseCtxCache {
		opts.init = append([]func(ctx context.Context) (context.Context, error){
			func(ctx context.Context) (context.Context, error) {
				return initCtxCachePreservingAuth(ctx), nil
			},
		}, opts.init...)
	}
			...
			if err != nil {
				...
				if hasErrOutput {
					...
					if output, err = runner.postProcess(ctx, output); err != nil {
						logs.ErrorContextf(ctx, "postProcess failed after returning error output: %v", err)
					}
				}
			}
		}()
...
func (r *nodeRunner[O]) invoke(ctx context.Context, input map[string]any, opts ...O) (output map[string]any, err error) {
	...
	for {
		...
		if err != nil {
			...
			logs.ErrorContextf(ctx, "[invoke] node %s ID %s failed on %d attempt, err: %v", r.nodeName, r.nodeKey, n, err)
...
func (r *nodeRunner[O]) stream(ctx context.Context, input map[string]any, opts ...O) (output *schema.StreamReader[map[string]any], err error) {
	...
	for {
		...
		if err != nil {
			...
			logs.ErrorContextf(ctx, "[stream] node %s ID %s failed on %d attempt, err: %v", r.nodeName, r.nodeKey, n, err)
			if r.maxRetry > n {
				n++
				if exeCtx := execute.GetExeCtx(ctx); exeCtx != nil && exeCtx.NodeCtx != nil {
					exeCtx.CurrentRetryCount++
...
func (r *nodeRunner[O]) collect(ctx context.Context, input *schema.StreamReader[map[string]any], opts ...O) (output map[string]any, err error) {
	...
	for {
		...
		if err != nil {
			...
			logs.ErrorContextf(ctx, "[collect] node %s ID %s failed on %d attempt, err: %v", r.nodeName, r.nodeKey, n, err)
			if r.maxRetry > n {
				n++
				if exeCtx := execute.GetExeCtx(ctx); exeCtx != nil && exeCtx.NodeCtx != nil {
					exeCtx.CurrentRetryCount++
...
func (r *nodeRunner[O]) transform(ctx context.Context, input *schema.StreamReader[map[string]any], opts ...O) (output *schema.StreamReader[map[string]any], err error) {
	...
	for {
		...
		if err != nil {
			...
			logs.ErrorContextf(ctx, "[transform] node %s ID %s failed on %d attempt, err: %v", r.nodeName, r.nodeKey, n, err)
			if r.maxRetry > n {
				n++
				if exeCtx := execute.GetExeCtx(ctx); exeCtx != nil && exeCtx.NodeCtx != nil {
					exeCtx.CurrentRetryCount++
...
func parseDefaultOutput(ctx context.Context, data string, schema_ map[string]*vo.TypeInfo) (map[string]any, error) {
	...
	if ws != nil {
		logs.WarnContextf(ctx, "convert output warnings: %v", *ws)
	}
...
func preTypeConverter(inTypes map[string]*vo.TypeInfo) func(ctx context.Context, in map[string]any) (map[string]any, error) {
	return func(ctx context.Context, in map[string]any) (map[string]any, error) {
		...
		if ws != nil {
			logs.WarnContextf(ctx, "convert inputs warnings: %v", *ws)
		}
