...
func (i *invokableWorkflow) InvokableRun(ctx context.Context, argumentsInJSON string, opts ...tool.Option) (
	...
	if previouslyInterrupted && rInfo.ExecuteID != previousExecuteID {
		logs.InfoContextf(ctx,"previous interrupted call ID: %s, previous execute ID: %d, current execute ID: %d. Not resuming, interrupt immediately", callID, previousExecuteID, rInfo.ExecuteID)
