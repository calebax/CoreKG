...
import (
	...
	cloudworkflow "github.com/insmtx/corekg/apps/workflow/api/model/workflow"
	...
	"github.com/insmtx/corekg/apps/workflow/domain/workflow/internal/repo"
...
func (i *impl) GetChatFlowRole(ctx context.Context, workflowID int64, version string) (*entity.ChatFlowRole, error) {
	...
	if !isExist {
		logs.WarnContextf(ctx, "chat flow role not exist, workflow id %v, version %v", workflowID, version)
...
func (i *impl) PublishChatFlowRole(ctx context.Context, policy *vo.PublishRolePolicy) error {
	if policy.WorkflowID == 0 || policy.CreatorID == 0 || policy.Version == "" {
		logs.ErrorContextf(ctx, "invalid publish role policy, workflow id %v, creator id %v should not be zero, version %v should not be empty", policy.WorkflowID, policy.CreatorID, policy.Version)
	...
	if !isExist {
		logs.ErrorContextf(ctx, "get draft chat flow role nil, workflow id %v", policy.WorkflowID)
...
func (i *impl) CopyWorkflow(ctx context.Context, workflowID int64, policy vo.CopyWorkflowPolicy) (*entity.Workflow, error) {
	...
	if wf.Mode == cloudworkflow.WorkflowMode_ChatFlow {
		...
		if !isExist {
			logs.ErrorContextf(ctx, "get draft chat flow role nil, workflow id %v", workflowID)
