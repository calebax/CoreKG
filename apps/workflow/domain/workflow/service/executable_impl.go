...
import (
	...
	workflowapimodel "github.com/insmtx/corekg/apps/workflow/api/model/workflow"
	...
	"github.com/insmtx/corekg/apps/workflow/domain/workflow/internal/canvas/adaptor"
	"github.com/insmtx/corekg/apps/workflow/domain/workflow/internal/compose"
...
func (i *impl) AsyncExecute(ctx context.Context, config workflowModel.ExecuteConfig, input map[string]any) (int64, error) {
	...
	if config.Mode == workflowModel.ExecuteModeDebug {
		if err = i.repo.SetTestRunLatestExeID(ctx, wfEntity.ID, config.Operator, executeID); err != nil {
			logs.ErrorContextf(ctx, "failed to set test run latest exe id: %v", err)
		}
	}
...
func (i *impl) handleHistory(ctx context.Context, config *workflowModel.ExecuteConfig, input map[string]any, historyRounds int64, shouldFetchConversationByName bool) error {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "failed to prefetch chat history: %v", err)
	}
...
func (i *impl) AsyncExecuteNode(ctx context.Context, nodeID string, config workflowModel.ExecuteConfig, input map[string]any) (int64, error) {
	...
	if config.Mode == workflowModel.ExecuteModeNodeDebug {
		if err = i.repo.SetNodeDebugLatestExeID(ctx, wfEntity.ID, nodeID, config.Operator, executeID); err != nil {
			logs.ErrorContextf(ctx, "failed to set node debug latest exe id: %v", err)
		}
	}
...
func (i *impl) GetLatestTestRunInput(ctx context.Context, wfID int64, userID int64) (*entity.NodeExecution, bool, error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[GetLatestTestRunInput] failed to get node execution from redis, wfID: %d, err: %v", wfID, err)
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[GetLatestTestRunInput] failed to get node execution, exeID: %d, err: %v", exeID, err)
		return nil, false, nil
	}
...
func (i *impl) GetLatestNodeDebugInput(ctx context.Context, wfID int64, nodeID string, userID int64) (
	*entity.NodeExecution, *entity.NodeExecution, bool, error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[GetLatestNodeDebugInput] failed to get node execution from redis, wfID: %d, nodeID: %s, err: %v",
			wfID, nodeID, err)
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[GetLatestNodeDebugInput] failed to get node execution, exeID: %d, nodeID: %s, err: %v",
			exeID, nodeID, err)
		return nil, nil, false, nil
	}
...
func (i *impl) prefetchChatHistory(ctx context.Context, config workflowModel.ExecuteConfig, historyRounds int64) ([]*crossmessage.WfMessage, []*schema.Message, error) {
	...
	if sectionID == nil {
		logs.WarnContextf(ctx, "SectionID is nil, skipping chat history")
		return nil, nil, nil
	}

	if convID == nil || *convID == 0 {
		logs.WarnContextf(ctx, "ConversationID is 0 or nil, skipping chat history")
		return nil, nil, nil
	}
	...
	if appID != nil {
		...
		logs.WarnContextf(ctx, "AppID and AgentID are both nil, skipping chat history")
		return nil, nil, nil
	}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "failed to get latest run ids: %v", err)
		return nil, nil, err
	}
	if len(runIds) <= 1 {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "failed to get messages by run ids: %v", err)
		return nil, nil, err
	}
