...
func (d *eventbusImpl) PublishResources(ctx context.Context, event *entity.ResourceDomainEvent) error {
	...
	if defaultResourceHandler != nil {
		...
		if err == nil {
			...
			logs.InfoContextf(ctx, "Sync PublishResources success: %s", string(json))

			return nil
		}

		logs.WarnContextf(ctx, "Sync PublishResources indexResources error: %s", err.Error())
	}
	...
	logs.InfoContextf(ctx,"PublishResources success: %s", string(bytes))
...
func (d *eventbusImpl) PublishProject(ctx context.Context, event *entity.ProjectDomainEvent) error {
	...
	if defaultProjectHandle != nil {
		...
		if err == nil {
			json, _ := sonic.Marshal(event)
			logs.InfoContextf(ctx, "Sync PublishProject success: %s", string(json))
			return nil
		}
		logs.WarnContextf(ctx, "Sync PublishProject indexProject error: %s", err.Error())
	}
	...
	logs.InfoContextf(ctx,"PublishProject success: %s", string(bytes))
	return d.producer.Send(ctx, bytes)
}
