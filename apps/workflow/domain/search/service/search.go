...
import (
	...
	model "github.com/insmtx/corekg/apps/workflow/crossdomain/search/model"
...
func (s *searchImpl) SearchProjects(ctx context.Context, req *searchEntity.SearchProjectsRequest) (resp *searchEntity.SearchProjectsResponse, err error) {
	logs.DebugContextf(ctx, "[SearchProjects] search : %s", conv.DebugJsonToStr(req))
	...
	// 搜索名称时 改为使用match搜索 NewContainsQuery为使用倒排索引模糊匹配
	if req.Name != "" {
		searchReq.Query.Bool.Must = append(searchReq.Query.Bool.Must,
			// es.NewContainsQuery("name", req.Name))
			es.NewMatchQuery("name", req.Name))
	}
	...
	if err != nil {
		logs.DebugContextf(ctx, "[Serarch.DO] err : %v", err)
		return nil, err
	}
...
func (s *searchImpl) SearchResources(ctx context.Context, req *searchEntity.SearchResourcesRequest) (resp *searchEntity.SearchResourcesResponse, err error) {
	logs.DebugContextf(ctx, "[SearchResources] search : %s", conv.DebugJsonToStr(req))
	searchReq := &es.Request{
		Query: &es.Query{
			Bool: &es.BoolQuery{},
		},
	...
	if len(req.ResTypeFilter) == 3 {
		resType := req.ResTypeFilter[0]
		resSubType := int(req.ResTypeFilter[2])
