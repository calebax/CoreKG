...
func loadPluginProductMeta(ctx context.Context, basePath string) (err error) {
	...
	for _, m := range pluginsMeta {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "plugin manifest validates failed, err=%v", err)
			continue
		}
		...
		if err != nil {
			logs.ErrorContextf(ctx, "load file '%s', err=%v", docPath, err)
			continue
		}
		...
		if err != nil {
			logs.ErrorContextf(ctx, "the openapi3 doc '%s' validates failed, err=%v", m.OpenapiDocFile, err)
			continue
		}
		...
		if pluginProducts[m.PluginID] != nil {
			logs.ErrorContextf(ctx, "duplicate plugin id '%d'", m.PluginID)
			continue
		}
		...
		for _, t := range m.Tools {
			...
			if ok {
				logs.ErrorContextf(ctx, "duplicate tool id '%d'", t.ToolID)
				continue
			}
			...
			op, ok := apis[api]
			if !ok {
				logs.ErrorContextf(ctx, "api '[%s]:%s' not found in doc '%s'", api.Method, api.SubURL, docPath)
				continue
			}
			if err = op.Validate(ctx); err != nil {
				logs.ErrorContextf(ctx, "the openapi3 operation of tool '[%s]:%s' in '%s' validates failed, err=%v",
...
func checkPluginMetaInfo(ctx context.Context, m *pluginProductMeta) (continued bool) {
	...
	if !semver.IsValid(m.Version) {
		logs.ErrorContextf(ctx, "invalid version '%s'", m.Version)
		return false
	}
	if m.PluginID <= 0 {
		logs.ErrorContextf(ctx, "invalid plugin id '%d'", m.PluginID)
		return false
	}
	...
	if m.PluginType != common.PluginType_PLUGIN {
		logs.ErrorContextf(ctx, "invalid plugin type '%s'", m.PluginType)
		return false
	}
