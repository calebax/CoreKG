...
import (
	...
	api "github.com/insmtx/corekg/apps/workflow/api/model/plugin_develop/common"
	...
	variables "github.com/insmtx/corekg/apps/workflow/crossdomain/variables/model"
...
func groupedKeysByLocation(ctx context.Context, apiSchema *model.Openapi3Operation) groupedKeys {
	...
	for _, paramRef := range paramRefs {
		...
		default:
			logs.WarnContextf(ctx, "[groupedKeysByLocation] unsupported parameter location '%s' in api schema, name=%s", valueSchema.In, valueSchema.Name)
			continue
		}
	}
...
func (i *InvocationArgs) groupedRequestArgs(ctx context.Context, args map[string]any) {
	...
	for k, v := range args {
		if _, ok := groupedKeySchema.HeaderKeys[k]; ok {
			...
			logs.WarnContextf(ctx, "[groupedRequestArgs] unsupported parameter key '%s' in api schema", k)
		}
	}
	...
	for location, params := range commonParams {
		for _, param := range params {
			...
			case consts.ParamInBody:
				dic = i.Body
			default:
				logs.WarnContextf(ctx, "unsupported common parameter location '%s' in api schema, name=%s", location, param.Name)
			}
...
func setParameterDefaultValues(ctx context.Context, dic map[string]any, paramSchema map[string]*openapi3.Parameter, projectInfo *model.ProjectInfo, userID string) (map[string]any, error) {
	for key, valueSchema := range paramSchema {
		if valueSchema.Schema == nil || valueSchema.Schema.Value == nil {
			logs.WarnContextf(ctx, "[setParameterDefaultValues] parameter '%s' schema is nil", key)
			continue
		}
		...
		if _, ok := dic[key]; !ok {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "get default value failed, key=%s, err=%v", key, err)
				return nil, err
			}
...
func getDefaultValue(ctx context.Context, schema *openapi3.Schema, info *model.ProjectInfo, userID string) (any, error) {
	...
	if !ok {
		logs.ErrorContextf(ctx, "invalid variable_ref type '%T'", vn)
		return nil, nil
	}
