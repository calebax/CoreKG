...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/internal/encoder"
...
func (h *httpCallImpl) Do(ctx context.Context, args *InvocationArgs) (request string, resp string, err error) {
	...
	logs.DebugContextf(ctx, "[execute] url=%s, header=%s, method=%s, body=%s",
	...
	logs.DebugContextf(ctx, "[execute] status=%s, response=%s", httpResp.Status(), httpResp.String())
