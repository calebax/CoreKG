...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/api/model/plugin_develop/common"
	...
	searchModel "github.com/insmtx/corekg/apps/workflow/crossdomain/search/model"
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/dto"
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/entity"
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/internal/openapi"
...
func (p *pluginServiceImpl) CreateDraftPlugin(ctx context.Context, req *dto.CreateDraftPluginRequest) (pluginID int64, err error) {
	...
	if req.SourceType == "" {
		...
		pl.SourceType = ptr.Of(req.SourceType)
	}
...
func (p *pluginServiceImpl) ListDraftPlugins(ctx context.Context, req *dto.ListDraftPluginsRequest) (resp *dto.ListDraftPluginsResponse, err error) {
	...
	for _, pl := range res.Data {
		...
		if !exist {
			logs.WarnContextf(ctx, "draft plugin not exist, pluginID=%d", pl.ResID)
			continue
		}
...
func (p *pluginServiceImpl) ConvertToOpenapi3Doc(ctx context.Context, req *dto.ConvertToOpenapi3DocRequest) (resp *dto.ConvertToOpenapi3DocResponse) {
	...
	defer func() {
		if err != nil {
			logs.ErrorContextf(ctx,"ConvertToOpenapi3Doc failed, err=%s", err)
