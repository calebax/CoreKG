...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/crossdomain/plugin/model"
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/dto"
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/encrypt"
...
func (p *pluginServiceImpl) processOAuthAccessToken(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			logs.WarnContextf(ctx, "DeleteInactiveAuthorizationCodeTokens failed, err=%v", err)
		}

		err = p.oauthRepo.DeleteExpiredAuthorizationCodeTokens(ctx, now.UnixMilli(), deleteLimit)
		if err != nil {
			logs.WarnContextf(ctx, "DeleteExpiredAuthorizationCodeTokens failed, err=%v", err)
		}

		refreshTokenList, err := p.oauthRepo.GetAuthorizationCodeRefreshTokens(ctx, now.UnixMilli(), refreshLimit)
		if err != nil {
			logs.ErrorContextf(ctx, "GetAuthorizationCodeRefreshTokens failed, err=%v", err)
		...
		if len(expired) > 0 {
			...
			if err != nil {
				logs.WarnContextf(ctx, "BatchDeleteAuthorizationCodeByIDs failed, err=%v", err)
			}
		}
...
func (p *pluginServiceImpl) refreshToken(ctx context.Context, info *dto.AuthorizationCodeInfo) {
	...
	if err != nil {
		logs.WarnContextf(ctx, "refreshToken failed, recordID=%d, err=%v", info.RecordID, err)
	...
	if err != nil {
		logs.InfoContextf(ctx, "UpsertAuthorizationCode failed, recordID=%d, err=%v", info.RecordID, err)
		p.refreshTokenFailedHandler(ctx, info.RecordID, err)
		return
	}
}
...
func (p *pluginServiceImpl) refreshTokenFailedHandler(ctx context.Context, recordID int64, err error) {
	...
	logs.ErrorContextf(ctx, "refreshToken exceeds max failed times, recordID=%d, err=%v", recordID, err)

	failedCache.Delete(recordID)

	err_ := p.oauthRepo.BatchDeleteAuthorizationCodeByIDs(ctx, []int64{recordID})
	if err_ != nil {
		logs.ErrorContextf(ctx, "BatchDeleteAuthorizationCodeByIDs failed, recordID=%d, err=%v", recordID, err_)
	}
}
...
func (p *pluginServiceImpl) getAccessTokenByAuthorizationCode(ctx context.Context, ci *dto.AuthorizationCodeInfo) (accessToken string, err error) {
	...
	if now-info.LastActiveAtMS > time.Minute.Milliseconds() { // don't update too frequently
		...
		if err != nil {
			logs.WarnContextf(ctx, "UpdateAuthorizationCodeLastActiveAt failed, userID=%s, pluginID=%d, isDraft=%t, err=%v",
...
func (p *pluginServiceImpl) GetAgentPluginsOAuthStatus(ctx context.Context, userID, agentID int64) (status []*dto.AgentPluginOAuthStatus, err error) {
	...
	for _, plugin := range plugins {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "getPluginOAuthStatus failed, pluginID=%d, err=%v", plugin.ID, err)
			continue
		}
