...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/crossdomain/database/model"
	database "github.com/insmtx/corekg/apps/workflow/crossdomain/database/model"
	crossvariables "github.com/insmtx/corekg/apps/workflow/crossdomain/variables"
	entity2 "github.com/insmtx/corekg/apps/workflow/domain/memory/database/entity"
	"github.com/insmtx/corekg/apps/workflow/domain/memory/database/internal/convertor"
	"github.com/insmtx/corekg/apps/workflow/domain/memory/database/internal/dal/query"
	"github.com/insmtx/corekg/apps/workflow/domain/memory/database/internal/physicaltable"
	"github.com/insmtx/corekg/apps/workflow/domain/memory/database/internal/sheet"
	"github.com/insmtx/corekg/apps/workflow/domain/memory/database/repository"
	"github.com/insmtx/corekg/apps/workflow/infra/idgen"
	"github.com/insmtx/corekg/apps/workflow/infra/rdb"
	entity3 "github.com/insmtx/corekg/apps/workflow/infra/rdb/entity"
...
func (d databaseService) CreateDatabase(ctx context.Context, req *CreateDatabaseRequest) (*CreateDatabaseResponse, error) {
	...
	defer func() {
		if r := recover(); r != nil {
			e := tx.Rollback()
			if e != nil {
				logs.ErrorContextf(ctx, "rollback failed, err=%v", e)
			}
...
func (d databaseService) DeleteDatabase(ctx context.Context, req *DeleteDatabaseRequest) error {
	...
	if draftInfo.ActualTableName != "" {
		...
		if err != nil {
			logs.ErrorContextf(ctx,"drop draft physical table failed: %v, table_name=%s", err, draftInfo.ActualTableName)
		}
	}
	...
	if onlineInfo.ActualTableName != "" {
		...
		if err != nil {
			logs.ErrorContextf(ctx,"drop online physical table failed: %v, table_name=%s", err, onlineInfo.ActualTableName)
		}
	}
...
func (d databaseService) UpdateDatabaseRecord(ctx context.Context, req *UpdateDatabaseRecordRequest) error {
	...
	for _, record := range req.Records {
		...
		for fieldName, valueStr := range record {
			...
			if err != nil {
				logs.WarnContextf(ctx,"convert value failed for field %s: %v, using original value", fieldName, err)
...
func (d databaseService) executeInsertSQL(ctx context.Context, req *ExecuteSQLRequest, physicalTableName string, tableInfo *entity2.Database) (*entity3.ResultSet, error) {
	...
	for index, upsertRow := range req.UpsertRows {
		...
		for _, record := range upsertRow.Records {
			...
			if err != nil {
				logs.WarnContextf(ctx,"convert value failed: %v, using original value", err)
...
func (d databaseService) DeleteDatabaseByAppID(ctx context.Context, req *DeleteDatabaseByAppIDRequest) (*DeleteDatabaseByAppIDResponse, error) {
	...
	for _, physical := range onlinePhysicals {
		...
		if err != nil {
			logs.ErrorContextf(ctx,"drop online physical table failed: %v, table_name=%s", err, physical)
		}
	}
	for _, physical := range draftPhysicals {
		...
		if err != nil {
			logs.ErrorContextf(ctx,"drop draft physical table failed: %v, table_name=%s", err, physical)
		}
	}
