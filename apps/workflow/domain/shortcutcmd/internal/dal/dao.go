...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/domain/shortcutcmd/entity"
	"github.com/insmtx/corekg/apps/workflow/domain/shortcutcmd/internal/dal/model"
	"github.com/insmtx/corekg/apps/workflow/domain/shortcutcmd/internal/dal/query"
...
func (dao *ShortCutCmdDAO) Create(ctx context.Context, shortcut *entity.ShortcutCmd) (*entity.ShortcutCmd, error) {
	...
	logs.InfoContextf(ctx, "ShortcutCommand %v, err:%v", conv.DebugJsonToStr(createPO), err)
	if createErr != nil {
...
func (dao *ShortCutCmdDAO) PublishCMDs(ctx context.Context, objID int64, cmdIDs []int64) error {
	...
	defer func() {
		if tx.Error != nil {
			rbErr := tx.Rollback()
			if rbErr != nil {
				logs.ErrorContextf(ctx, "rollback failed, err:%v", rbErr)
			}
		}
		cErr := tx.Commit()
		if cErr != nil {
			logs.ErrorContextf(ctx, "commit failed, err:%v", cErr)
		}
	}()
	...
	for _, item := range draftCmds {
		...
		logs.InfoContextf(ctx, "publish cmd %v, err:%v", conv.DebugJsonToStr(item), opErr)
		if opErr != nil {
			return opErr
		}
	}
