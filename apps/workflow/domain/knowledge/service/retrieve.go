...
import (
	...
	sqlparsercontract "github.com/insmtx/corekg/apps/workflow/infra/sqlparser"
	"github.com/insmtx/corekg/apps/workflow/pkg/errorx"
	"github.com/insmtx/corekg/apps/workflow/pkg/lang/ptr"
	"github.com/insmtx/corekg/apps/workflow/pkg/lang/sets"
...
func (k *knowledgeSVC) Retrieve(ctx context.Context, request *RetrieveRequest) (response *RetrieveResponse, err error) {
	...
	r, err := chain.
		...
		Compile(ctx)
	if err != nil {
		logs.ErrorContextf(ctx, "compile chain failed: %v", err)
	...
	if err != nil {
		logs.ErrorContextf(ctx, "invoke chain failed: %v", err)
...
func (k *knowledgeSVC) newRetrieveContext(ctx context.Context, req *RetrieveRequest) (*RetrieveContext, error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "prepare rag documents failed: %v", err)
		return nil, err
	}
	if len(enableDocs) == 0 {
...
func (k *knowledgeSVC) prepareRAGDocuments(ctx context.Context, documentIDs []int64, knowledgeIDs []int64) ([]*model.KnowledgeDocument, []*model.Knowledge, error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "filter enable knowledge failed: %v", err)
	...
	if err != nil {
		logs.ErrorContextf(ctx, "find document by condition failed: %v", err)
		return nil, nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
...
func (k *knowledgeSVC) queryRewriteNode(ctx context.Context, req *RetrieveContext) (newRetrieveContext *RetrieveContext, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "rewrite query failed: %v", err)
		return req, nil
	}
	// Rewrite completed
...
func (k *knowledgeSVC) vectorRetrieveNode(ctx context.Context, req *RetrieveContext) (retrieveResult []*schema.Document, err error) {
	...
	if manager == nil {
		logs.ErrorContextf(ctx, "err:%s", errorx.New(errno.ErrKnowledgeSearchStoreCode, errorx.KV("msg", "未实现vectorStore")).Error())
		return nil, nil
	}

	retrieveResult, err = k.retrieveChannels(ctx, req, manager)
	if err != nil {
		logs.ErrorContextf(ctx, "retrieveChannels err:%s", err.Error())
	}
...
func (k *knowledgeSVC) esRetrieveNode(ctx context.Context, req *RetrieveContext) (retrieveResult []*schema.Document, err error) {
	...
	if manager == nil {
		logs.ErrorContextf(ctx, "err:%s", errorx.New(errno.ErrKnowledgeSearchStoreCode, errorx.KV("msg", "未实现esStore")).Error())
		return nil, nil
	}
	...
	if hasTable && req.Strategy.EnableNL2SQL {
		...
		for i := range tableDocs {
			...
			eg.Go(func() error {
				...
				if execErr != nil {
					logs.ErrorContextf(ctx, "nl2sql exec failed: %v", execErr)
		...
		err = eg.Wait()
		if err != nil {
			logs.ErrorContextf(ctx, "nl2sql exec failed: %v", err)
			return nil, nil
		}
		return res, nil
	} else {
...
func (k *knowledgeSVC) nl2SqlExec(ctx context.Context, doc *model.KnowledgeDocument, retrieveCtx *RetrieveContext, opts []nl2sql.Option) (
	...
	if err != nil {
		logs.ErrorContextf(ctx, "nl2sql failed: %v", err)
		return nil, err
	}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "parse sql failed: %v", err)
		return nil, err
	}
	// Execute sql
	...
	if err != nil {
		logs.ErrorContextf(ctx, "execute sql failed: %v", err)
		return nil, err
	}
	for i := range resp.ResultSet.Rows {
		...
		if !ok {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "marshal sql resp failed: %v", err)
				return nil, err
			}
...
func (k *knowledgeSVC) reRankNode(ctx context.Context, resultMap map[string]any) (retrieveResult []*schema.Document, err error) {
	...
	if !ok {
		logs.ErrorContextf(ctx, "retrieve context is not found")
	...
	if !ok {
		logs.ErrorContextf(ctx, "vector retrieve result is not found")
	...
	if !ok {
		logs.ErrorContextf(ctx, "es retrieve result is not found")
	...
	if !ok {
		logs.ErrorContextf(ctx, "nl2sql retrieve result is not found")
	...
	if err != nil {
		logs.ErrorContextf(ctx, "rerank failed: %v", err)
		return nil, err
	}
...
func (k *knowledgeSVC) packResults(ctx context.Context, retrieveResult []*schema.Document) (results []*knowledgeModel.RetrieveSlice, err error) {
	...
	for _, doc := range retrieveResult {
		if len(doc.ID) == 0 {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "convert id failed: %v", err)
	...
	if err != nil {
		logs.ErrorContextf(ctx, "mget slices failed: %v", err)
	...
	if err != nil {
		logs.ErrorContextf(ctx, "mget documents failed: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	...
	for docID, slices := range slicesInTable {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "select table data failed: %v", err)
			return nil, err
		}
	}
	for i := range slices {
		...
		if len(docUri) != 0 {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "get object url failed: %v", err)
	...
	if err != nil {
		logs.WarnContextf(ctx, "increment hit count failed: %v", err)
	}
	return results, nil
}
