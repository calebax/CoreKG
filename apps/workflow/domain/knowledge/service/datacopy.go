...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/domain/datacopy"
	copyEntity "github.com/insmtx/corekg/apps/workflow/domain/datacopy/entity"
...
func (k *knowledgeSVC) copyDo(ctx context.Context, copyCtx *knowledgeCopyCtx) (*CopyKnowledgeResponse, error) {
	...
	defer func() {
		if e := recover(); e != nil {
			logs.ErrorContextf(ctx, "copy knowledge failed, err: %v", e)
		...
		if err != nil {
			...
			if deleteErr != nil {
				logs.ErrorContextf(ctx, "delete knowledge failed, err: %v", deleteErr)
			}
			if len(copyCtx.NewRDBTableNames) != 0 {
				for i := range copyCtx.NewRDBTableNames {
					...
					if dropErr != nil {
						logs.ErrorContextf(ctx, "[copyDo] drop table failed, err: %v", dropErr)
					}
				}
			}
			...
			if err != nil {
				logs.ErrorContextf(ctx, "update copy task failed, err: %v", err)
			}
		}
	}()
	...
	if err != nil {
		logs.ErrorContextf(ctx, "copy knowledge failed, err: %v", err)
		return nil, err
	}
	...
	err = crossdatacopy.DefaultSVC().UpdateCopyTask(ctx, &datacopy.UpdateCopyTaskReq{Task: copyCtx.CopyTask})
	if err != nil {
		logs.WarnContextf(ctx, "update copy task failed, err: %v", err)
	}
	return &CopyKnowledgeResponse{
...
func (k *knowledgeSVC) copyKnowledgeDocuments(ctx context.Context, copyCtx *knowledgeCopyCtx) (err error) {
	...
	if len(documents) == 0 {
		logs.InfoContextf(ctx, "knowledge %d has no document", copyCtx.OriginData.ID)
		return nil
	}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "find target document failed, err: %v", err)
		return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "gen document id failed, err: %v", err)
	...
	for i := range documents {
		...
		eg.Go(func() error {
			...
			if cpErr != nil {
				...
				mu.Unlock()
				logs.ErrorContextf(ctx, "copy document failed, src document id: %d, new id: %d, err: %v", doc.ID, newID, err)
				return cpErr
			}
			return nil
		})
	...
	if err := eg.Wait(); err != nil {
		logs.ErrorContextf(ctx, "copy document failed, document ids: %v, first-err: %v", failList, err)
...
func (k *knowledgeSVC) copyDocument(ctx context.Context, copyCtx *knowledgeCopyCtx, doc *model.KnowledgeDocument, newDocID int64) (err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "map search fields failed, err: %v", err)
	...
	defer func() {
		...
		if updateErr != nil {
			logs.ErrorContextf(ctx, "update document status failed, err: %v", updateErr)
		}
	}()
	batchSize := 100
...
func (k *knowledgeSVC) createTable(ctx context.Context, doc *model.KnowledgeDocument) error {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "create table failed, err: %v", err)
		return err
	}
