...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/domain/knowledge/internal/events"
	"github.com/insmtx/corekg/apps/workflow/domain/knowledge/processor/impl"
	"github.com/insmtx/corekg/apps/workflow/domain/knowledge/repository"
	"github.com/insmtx/corekg/apps/workflow/infra/cache"
	"github.com/insmtx/corekg/apps/workflow/infra/document/messages2query"
	"github.com/insmtx/corekg/apps/workflow/infra/document/nl2sql"
	...
	"github.com/insmtx/corekg/apps/workflow/infra/document/rerank"
...
func (k *knowledgeSVC) DeleteKnowledge(ctx context.Context, request *DeleteKnowledgeRequest) error {
	...
	if knModel.FormatType == int32(knowledgeModel.DocumentTypeTable) {
		for _, doc := range docs {
			...
			if doc.TableInfo != nil {
				...
				if err != nil {
					logs.WarnContextf(ctx, "[DeleteKnowledge] drop table failed, err %v", err)
				}
				if !resp.Success {
					logs.WarnContextf(ctx, "[DeleteKnowledge] drop table failed")
				}
			}
		}
	}
	...
	if err = k.documentRepo.DeleteDocuments(ctx, slices.Transform(docs, func(a *model.KnowledgeDocument) int64 {
		...
		logs.ErrorContextf(ctx, "[DeleteKnowledge] delete documents failed, err %v", err)
		return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
...
func (k *knowledgeSVC) UpdateDocument(ctx context.Context, request *UpdateDocumentRequest) error {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[UpdateDocument] update document failed, err: %v", err)
		return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	return nil
}
...
func (k *knowledgeSVC) DeleteDocument(ctx context.Context, request *DeleteDocumentRequest) error {
	...
	if doc == nil || doc.ID == 0 {
		logs.WarnContextf(ctx, "[DeleteDocument] document not found, doc_id: %d", request.DocumentID)
		return nil
	}
	...
	if doc.DocumentType == int32(knowledgeModel.DocumentTypeTable) && doc.TableInfo != nil {
		...
		if err != nil {
			logs.WarnContextf(ctx, "[DeleteDocument] drop table failed, err: %v", err)
			return errorx.New(errno.ErrKnowledgeCrossDomainCode, errorx.KV("msg", err.Error()))
		}
		if !resp.Success {
			logs.WarnContextf(ctx, "[DeleteDocument] drop table failed")
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[DeleteDocument] get document slice ids failed, err: %v", err)
		return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
...
func (k *knowledgeSVC) ListDocument(ctx context.Context, request *ListDocumentRequest) (response *ListDocumentResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "list document failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
...
func (k *knowledgeSVC) MGetDocumentProgress(ctx context.Context, request *MGetDocumentProgressRequest) (response *MGetDocumentProgressResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "mget document failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	...
	for i := range documents {
		...
		if documents[i].DocumentType == int32(knowledgeModel.DocumentTypeImage) && len(documents[i].URI) != 0 {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "get object url failed, err: %v", err)
		...
		if documents[i].Status == int32(entity.DocumentStatusEnable) || documents[i].Status == int32(entity.DocumentStatusFailed) {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "get progress from cache failed, err: %v", err)
...
func (k *knowledgeSVC) CreateSlice(ctx context.Context, request *CreateSliceRequest) (response *CreateSliceResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "find document failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	...
	if docInfo.DocumentType == int32(knowledgeModel.DocumentTypeTable) {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "FindSliceByCondition err:%v", err)
			return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
		}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "get slice by sequence failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	now := time.Now().UnixMilli()
	id, err := k.idgen.GenID(ctx)
	...
	if err != nil {
		logs.ErrorContextf(ctx, "fromModelDocument failed, err: %v", err)
		return nil, err
	}
	...
	if docInfo.DocumentType == int32(knowledgeModel.DocumentTypeTable) {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "insert data to table failed, err: %v", err)
			return nil, err
		}
	}
	err = k.sliceRepo.Create(ctx, &sliceInfo)
	if err != nil {
		logs.ErrorContextf(ctx, "create slice failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	body, err := sonic.Marshal(&indexSliceEvent)
	if err != nil {
		logs.ErrorContextf(ctx, "marshal event failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeParseJSONCode, errorx.KV("msg", err.Error()))
	}
	if err = k.producer.Send(ctx, body, eventbus.WithShardingKey(strconv.FormatInt(sliceInfo.DocumentID, 10))); err != nil {
		logs.ErrorContextf(ctx, "send message failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeMQSendFailCode, errorx.KV("msg", err.Error()))
	}
	if err = k.documentRepo.UpdateDocumentSliceInfo(ctx, docInfo.ID); err != nil {
		logs.ErrorContextf(ctx, "update document slice info failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
...
func (k *knowledgeSVC) UpdateSlice(ctx context.Context, request *UpdateSliceRequest) error {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "mget slice failed, err: %v", err)
		return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	if len(sliceInfo) != 1 {
	...
	if docInfo.DocumentType == int32(knowledgeModel.DocumentTypeTable) {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "upsert data to table failed, err: %v", err)
			return err
		}
	}
	err = k.sliceRepo.Update(ctx, sliceInfo[0])
	if err != nil {
		logs.ErrorContextf(ctx, "update slice failed, err: %v", err)
		return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	body, err := sonic.Marshal(&indexSliceEvent)
	if err != nil {
...
func (k *knowledgeSVC) DeleteSlice(ctx context.Context, request *DeleteSliceRequest) error {
	...
	if len(sliceInfo) != 1 {
		logs.WarnContextf(ctx, "slice not found, slice_id: %d", request.SliceID)
		return nil
	}
	...
	if docInfo.DocumentType == int32(knowledgeModel.DocumentTypeTable) {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "delete data failed, err: %v", err)
			return errorx.New(errno.ErrKnowledgeCrossDomainCode, errorx.KV("msg", err.Error()))
		}
	}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "delete slice failed, err: %v", err)
		return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
...
func (k *knowledgeSVC) ListSlice(ctx context.Context, request *ListSliceRequest) (response *ListSliceResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "get document failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "list slice failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	...
	if doc.DocumentType == int32(knowledgeModel.DocumentTypeTable) {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "select table data failed, err: %v", err)
			return nil, err
		}
	}
...
func (k *knowledgeSVC) CreateDocumentReview(ctx context.Context, request *CreateDocumentReviewRequest) (response *CreateDocumentReviewResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "get knowledge failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	if kn == nil {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "create review failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	for i := range reviews {
		review := reviews[i]
...
func (k *knowledgeSVC) MGetDocumentReview(ctx context.Context, request *MGetDocumentReviewRequest) (response *MGetDocumentReviewResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "mget review failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	for _, review := range reviews {
...
func (k *knowledgeSVC) SaveDocumentReview(ctx context.Context, request *SaveDocumentReviewRequest) error {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "get review failed, err: %v", err)
		return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	...
	if review.Status == int32(entity.ReviewStatus_Enable) && len(uri) > 0 {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "put object failed, err: %v", err)
		...
		if err != nil {
			logs.ErrorContextf(ctx, "update review chunk uri failed, err: %v", err)
			return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
		}
	}
	return nil
...
func (k *knowledgeSVC) fromModelKnowledge(ctx context.Context, knowledge *model.Knowledge) (*knowledgeModel.Knowledge, error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "get slice hit count failed, err: %v", err)
		return nil, errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
...
func (k *knowledgeSVC) getObjectURL(ctx context.Context, uri string) (string, error) {
	...
	if cmd.Err() != nil {
		...
		if errors.Is(cmd.Err(), cache.Nil) {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "[getObjectURL] set cache failed, %v", err)
			}
		}
		return url, nil
	}
	...
	for l := 0; l < counts; l += 100 {
		...
		for {
			...
			if err != nil {
				...
				logs.ErrorContextf(ctx, "[genMultiIDs] GenMultiIDs failed, retry %d/%d: %v", retryCount+1, maxRetries, err)
