...
func (c *customTableProcessor) BeforeCreate() error {
	if isTableAppend(c.Documents) {
		...
		if err != nil {
			logs.ErrorContextf(c.ctx, "find document failed, err: %v", err)
			return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
		}
		if len(tableDoc) == 0 {
			logs.ErrorContextf(c.ctx, "table doc not found")
		...
		if tableDoc[0].TableInfo == nil {
			logs.ErrorContextf(c.ctx, "table info not found")
		...
		if c.Documents[0].RawContent != "" {
			...
			if err != nil {
				logs.ErrorContextf(c.ctx, "put object failed, err: %v", err)
...
func (c *customTableProcessor) InsertDBModel() error {
	if isTableAppend(c.Documents) {
		...
		if err != nil {
			logs.ErrorContextf(c.ctx, "document set status err:%v", err)
			return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
		}
		return nil
	}
...
func (c *customTableProcessor) Indexing() error {
	...
	if isTableAppend(c.Documents) {
		...
		if err != nil {
			logs.ErrorContextf(c.ctx, "document indexing err:%v", err)
			return err
		}
	}
	return nil
