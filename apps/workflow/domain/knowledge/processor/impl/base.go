...
func (p *baseDocProcessor) BuildDBModel() error {
	...
	for i := range p.Documents {
		...
		if err != nil {
			logs.ErrorContextf(p.ctx, "gen id failed, err: %v", err)
...
func (p *baseDocProcessor) InsertDBModel() (err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "init tx failed, err: %v", err)
		return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
	}
	defer func() {
		if e := recover(); e != nil {
			logs.ErrorContextf(ctx, "panic: %v", e)
			err = errorx.New(errno.ErrKnowledgeSystemCode, errorx.KVf("msg", "panic: %v", e))
			tx.Rollback()
			return
		}
		if err != nil {
			logs.ErrorContextf(ctx, "InsertDBModel err: %v", err)
			tx.Rollback()
			if p.TableName != "" {
				deleteErr := p.deleteTable()
				if deleteErr != nil {
					logs.ErrorContextf(ctx, "delete table failed, err: %v", deleteErr)
					return
				}
			}
		} else {
...
func (p *baseDocProcessor) createTable() error {
	if len(p.Documents) == 1 && p.Documents[0].Type == knowledge.DocumentTypeTable {
		...
		if err != nil {
			logs.ErrorContextf(p.ctx, "gen ids failed, err: %v", err)
			return errorx.New(errno.ErrKnowledgeIDGenCode)
		}
		...
		if err != nil {
			logs.ErrorContextf(p.ctx, "create table failed, err: %v", err)
			return errorx.New(errno.ErrKnowledgeCrossDomainCode, errorx.KV("msg", err.Error()))
		}
...
func (p *baseDocProcessor) deleteTable() error {
	if len(p.Documents) == 1 && p.Documents[0].Type == knowledge.DocumentTypeTable {
		...
		if err != nil {
			logs.ErrorContextf(p.ctx, "[deleteTable] drop table failed, err: %v", err)
			return errorx.New(errno.ErrKnowledgeCrossDomainCode, errorx.KV("msg", err.Error()))
		}
	}
	return nil
...
func (p *baseDocProcessor) Indexing() error {
	...
	if err = p.producer.Send(p.ctx, body); err != nil {
		logs.ErrorContextf(p.ctx, "send message failed, err: %v", err)
