...
func (l *localTableProcessor) BeforeCreate() error {
	if isTableAppend(l.Documents) {
		...
		if err != nil {
			logs.ErrorContextf(l.ctx, "find document failed, err: %v", err)
			return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
		}

		if len(tableDoc) == 0 {
			logs.ErrorContextf(l.ctx, "table doc not found")
		...
		if tableDoc[0].TableInfo == nil {
			logs.ErrorContextf(l.ctx, "table info not found")
...
func (l *localTableProcessor) InsertDBModel() error {
	if isTableAppend(l.Documents) {
		...
		if err != nil {
			logs.ErrorContextf(l.ctx, "document set status err:%v", err)
			return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", err.Error()))
		}
		return nil
	}
