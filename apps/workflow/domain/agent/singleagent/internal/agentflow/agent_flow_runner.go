...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/pkg/urltobase64url"
)
	...
	safego.Go(ctx, func() {
		defer func() {
			if pe := recover(); pe != nil {
				logs.ErrorContextf(ctx, "[AgentRunner] StreamExecute recover, err: %v", pe)
...
func (r *AgentRunner) PreHandlerReq(ctx context.Context, req *AgentRequest) *AgentRequest {
	...
	logs.InfoContextf(ctx, "[AgentRunner] PreHandlerReq, req: %v", conv.DebugJsonToStr(req))
