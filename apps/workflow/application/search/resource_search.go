...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/api/model/resource"
	...
	"github.com/insmtx/corekg/apps/workflow/domain/search/entity"
	...
	if len(searchResp.Data) > 1 {
		for idx := range searchResp.Data[1:] {
			...
			tasks.Go(func() error {
				...
				if err != nil {
					logs.ErrorContextf(ctx, "[LibraryResourceList] packResource failed, will ignore resID: %d, Name : %s, resType: %d, err: %v",
...
func (s *SearchApplicationService) getResourceDefaultIconURL(ctx context.Context, tp common.ResType) string {
	...
	if !ok {
		logs.WarnContextf(ctx, "[getDefaultIconURL] don't have type: %d  default icon", tp)
...
func (s *SearchApplicationService) getURL(ctx context.Context, uri string) string {
	...
	if err != nil {
		logs.WarnContextf(ctx, "[getDefaultIconURLWitURI] GetObjectUrl failed, uri: %s, err: %v", uri, err)
...
func (s *SearchApplicationService) packUserInfo(ctx context.Context, ri *common.ResourceInfo, ownerID int64) *common.ResourceInfo {
	u, err := s.UserDomainSVC.GetUserProfiles(ctx, ownerID)
	if err != nil {
		logs.WarnContextf(ctx, "[LibraryResourceList] GetUserInfo failed, uid: %d, resID: %d, Name : %s, err: %v",
...
func (s *SearchApplicationService) packResource(ctx context.Context, doc *entity.ResourceDocument) (*common.ResourceInfo, error) {
	...
	if err != nil {
		logs.WarnContextf(ctx, "[packResource] GetDataInfo failed, resID: %d, Name : %s, resType: %d, err: %v",
...
func (s *SearchApplicationService) packAPPResources(ctx context.Context, resources []*entity.ResourceDocument) ([]*common.ProjectResourceGroup, error) {
	...
	for idx := range resources {
		...
		tasks.Go(func() error {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "packAPPResources failed, will ignore resID: %d, Name : %s, resType: %d, err: %v",
					v.ResID, v.GetName(), v.ResType, err)
				return err
			}
			...
			default:
				logs.WarnContextf(ctx, "unsupported resType: %d", v.ResType)
			}
...
func (s *SearchApplicationService) packProjectResource(ctx context.Context, resource *entity.ResourceDocument) (*common.ProjectResourceInfo, error) {
	...
	if resource.ResType == common.ResType_Knowledge {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "GetDataInfo failed, resID=%d, resType=%d, err=%v",
	...
	if resource.ResType == common.ResType_Plugin {
		...
		if err != nil {
			...
			if !errors.As(err, &e) {
				logs.ErrorContextf(ctx, "CheckPluginToolsDebugStatus failed, resID=%d, resType=%d, err=%v",
					resource.ResID, resource.ResType, err)
			} else {
				actions := slices.Clone(info.Actions)
				for _, a := range actions {
