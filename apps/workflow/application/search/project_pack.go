...
import (
	...
	appService "github.com/insmtx/corekg/apps/workflow/domain/app/service"
...
func (p *projectBase) GetUserInfo(ctx context.Context, userID int64) *common.User {
	u, err := p.SVC.UserDomainSVC.GetUserProfiles(ctx, userID)
	if err != nil {
		logs.ErrorContextf(ctx, "[projectBase-GetUserInfo] failed to get user info, user_id: %d, err: %v", userID, err)
		return nil
	}
...
func (p *agentPacker) GetPublishedInfo(ctx context.Context) *intelligence.IntelligencePublishInfo {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[agent-GetPublishedInfo]failed to get published info, agent_id: %d, err: %v", p.projectID, err)
	...
	for connectorID := range pubInfo.ConnectorID2PublishTime {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "failed to get connector by id: %d, err: %v", connectorID, err)
...
func (a *appPacker) GetPublishedInfo(ctx context.Context) *intelligence.IntelligencePublishInfo {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[app-GetPublishedInfo] failed to get published info, app_id=%d, err=%v", a.projectID, err)
		return nil
	}
	if !exist {
		return &intelligence.IntelligencePublishInfo{
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[app-GetPublishedInfo] failed to get connector info, app_id=%d, err=%v", a.projectID, err)
	} else {
