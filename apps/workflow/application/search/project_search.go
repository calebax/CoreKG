...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/api/model/marketplace/marketplace_common"
	"github.com/insmtx/corekg/apps/workflow/api/model/marketplace/product_common"
	"github.com/insmtx/corekg/apps/workflow/api/model/marketplace/product_public_api"
	"github.com/insmtx/corekg/apps/workflow/application/base/ctxutil"
	search2 "github.com/insmtx/corekg/apps/workflow/crossdomain/search/model"
...
func (s *SearchApplicationService) GetDraftIntelligenceList(ctx context.Context, req *intelligence.GetDraftIntelligenceListRequest) (
	...
	logs.DebugContextf(ctx, "[GetDraftIntelligenceList] searchResp.Data: %v", conv.DebugJsonToStr(searchResp.Data))
	if len(searchResp.Data) > 1 {
		for idx := range searchResp.Data[1:] {
			...
			tasks.Go(func() error {
				...
				if err != nil {
					logs.ErrorContextf(ctx, "[packIntelligenceData] failed id %v, type %d , name %s, err: %v", data.ID, data.Type, data.GetName(), err)
					return nil
				}
	...
	if len(searchResp.Data) != 0 {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "[packIntelligenceData] failed id %v, type %d , name %s, err: %v", searchResp.Data[0].ID, searchResp.Data[0].Type, searchResp.Data[0].GetName(), err)
		} else {
			lock.Lock()
...
func (s *SearchApplicationService) searchFavProjects(ctx context.Context, userID int64, req *product_public_api.GetUserFavoriteListV2Request) (*product_public_api.GetUserFavoriteListDataV2, error) {
	...
	for _, r := range res.Data {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "[pluginResourceToProductInfo] failed to get project info, id=%v, type=%d, err=%v",
				r.ID, r.Type, err)
			continue
		}
...
func (s *SearchApplicationService) getProjectDefaultIconURL(ctx context.Context, tp common.IntelligenceType) string {
	...
	if !ok {
		logs.WarnContextf(ctx, "[getProjectDefaultIconURL] don't have type: %d  default icon", tp)
