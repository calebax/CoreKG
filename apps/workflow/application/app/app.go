...
import (
	...
	intelligenceAPI "github.com/insmtx/corekg/apps/workflow/api/model/app/intelligence"
	"github.com/insmtx/corekg/apps/workflow/api/model/app/intelligence/common"
	taskStruct "github.com/insmtx/corekg/apps/workflow/api/model/app/intelligence/common"
	projectAPI "github.com/insmtx/corekg/apps/workflow/api/model/app/intelligence/project"
	publishAPI "github.com/insmtx/corekg/apps/workflow/api/model/app/intelligence/publish"
	taskAPI "github.com/insmtx/corekg/apps/workflow/api/model/app/intelligence/task"
	"github.com/insmtx/corekg/apps/workflow/api/model/data/database/table"
	"github.com/insmtx/corekg/apps/workflow/api/model/data/variable/project_memory"
	"github.com/insmtx/corekg/apps/workflow/api/model/playground"
	resourceAPI "github.com/insmtx/corekg/apps/workflow/api/model/resource"
	resourceCommon "github.com/insmtx/corekg/apps/workflow/api/model/resource/common"
	workflowAPI "github.com/insmtx/corekg/apps/workflow/api/model/workflow"
	...
	connectorModel "github.com/insmtx/corekg/apps/workflow/crossdomain/connector/model"
	...
	"github.com/insmtx/corekg/apps/workflow/domain/app/entity"
	"github.com/insmtx/corekg/apps/workflow/domain/app/repository"
	"github.com/insmtx/corekg/apps/workflow/domain/app/service"
	...
	"github.com/insmtx/corekg/apps/workflow/pkg/taskgroup"
...
func (a *APPApplicationService) DraftProjectDelete(ctx context.Context, req *projectAPI.DraftProjectDeleteRequest) (resp *projectAPI.DraftProjectDeleteResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "publish project '%d' failed, err=%v", req.ProjectID, err)
	}
...
func (a *APPApplicationService) deleteAPPResources(ctx context.Context, appID int64) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "delete app '%d' plugins failed, err=%v", appID, err)
	}

	err = memory.DatabaseApplicationSVC.DeleteDatabaseByAppID(ctx, appID)
	if err != nil {
		logs.ErrorContextf(ctx, "delete app '%d' databases failed, err=%v", appID, err)
	}

	err = a.variablesSVC.DeleteAllVariable(ctx, project_memory.VariableConnector_Project, conv.Int64ToStr(appID))
	if err != nil {
		logs.ErrorContextf(ctx, "delete app '%d' variables failed, err=%v", appID, err)
	}

	err = knowledge.KnowledgeSVC.DeleteAppKnowledge(ctx, &knowledge.DeleteAppKnowledgeRequest{AppID: appID})
	if err != nil {
		logs.ErrorContextf(ctx, "delete app '%d' knowledge failed, err=%v", appID, err)
	}

	err = workflow.SVC.DeleteWorkflowsByAppID(ctx, appID)
	if err != nil {
		logs.ErrorContextf(ctx, "delete app '%d' workflow failed, err=%v", appID, err)
	}
}
...
func (a *APPApplicationService) getAPPPublishConnectorList(ctx context.Context, appID int64) ([]*publishAPI.PublishConnectorInfo, error) {
	...
	for _, c := range res.Connectors {
		...
		default:
			logs.WarnContextf(ctx, "unsupported connector id '%v'", c.ID)
			continue
		}
...
func (a *APPApplicationService) ReportUserBehavior(ctx context.Context, req *playground.ReportUserBehaviorRequest) (resp *playground.ReportUserBehaviorResponse, err error) {
	...
	if err != nil {
		logs.WarnContextf(ctx, "publish project '%d' event failed err=%s", req.ResourceID, err)
	}
...
func (a *APPApplicationService) GetPublishRecordList(ctx context.Context, req *publishAPI.GetPublishRecordListRequest) (resp *publishAPI.GetPublishRecordListResponse, err error) {
	...
	for _, r := range records {
		...
		for _, c := range r.ConnectorPublishRecords {
			...
			if !exist {
				logs.ErrorContextf(ctx, "connector '%d' not exist", c.ConnectorID)
				continue
			}
...
func (a *APPApplicationService) ResourceCopyDispatch(ctx context.Context, req *resourceAPI.ResourceCopyDispatchRequest) (resp *resourceAPI.ResourceCopyDispatchResponse, err error) {
	...
	if handleErr != nil {
		logs.ErrorContextf(ctx, "copy resource failed, taskID=%s, err=%v", taskID, handleErr)
	}
...
func (a *APPApplicationService) GetOnlineAppData(ctx context.Context, req *projectAPI.GetOnlineAppDataRequest) (resp *projectAPI.GetOnlineAppDataResponse, err error) {
	...
	if err != nil {
		logs.WarnContextf(ctx, "get icon url failed with '%s', err=%v", app.GetIconURI(), err)
	}
...
func (a *APPApplicationService) getAPPUserInfo(ctx context.Context, userID int64) (userInfo *common.User) {
	ui, err := a.userSVC.GetUserProfiles(ctx, userID)
	if err != nil {
		logs.ErrorContextf(ctx, "GetUserInfo failed, userID=%d, err=%v", userID, err)
		return nil
	}
...
func (a *APPApplicationService) getAPPBasicInfo(ctx context.Context, draftAPP *entity.APP) (info *common.IntelligenceBasicInfo, published bool, err error) {
	...
	if err != nil {
		logs.WarnContextf(ctx, "get icon url failed with '%s', err=%v", draftAPP.GetIconURI(), err)
	}
