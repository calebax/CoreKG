...
import (
	...
	pluginEntity "github.com/insmtx/corekg/apps/workflow/domain/plugin/entity"
	shortcutCMDEntity "github.com/insmtx/corekg/apps/workflow/domain/shortcutcmd/entity"
	workflowEntity "github.com/insmtx/corekg/apps/workflow/domain/workflow/entity"
...
func (s *SingleAgentApplicationService) fetchShortcutCMD(ctx context.Context, agentInfo *entity.SingleAgent) ([]*playground.ShortcutCommand, error) {
	...
	logs.InfoContextf(ctx, "fetchShortcutCMD cmdDOs = %v, err = %v", conv.DebugJsonToStr(cmdDOs), err)
...
func (s *SingleAgentApplicationService) pluginInfoDo2Vo(ctx context.Context, pluginInfos []*pluginEntity.PluginInfo) map[int64]*playground.PluginDetal {
	return slices.ToMap(pluginInfos, func(v *pluginEntity.PluginInfo) (int64, *playground.PluginDetal) {
		...
		if e.IconURL != nil {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "get icon url failed, err = %v", err)
			}
		}
