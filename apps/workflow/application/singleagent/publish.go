...
func (s *SingleAgentApplicationService) PublishAgent(ctx context.Context, req *developer_api.PublishDraftBotRequest) (*developer_api.PublishDraftBotResponse, error) {
	...
	for _, connectorID := range connectorIDs {
		tasks.Go(func() error {
			...
			if err != nil {
				logs.WarnContextf(ctx, "create single agent failed: %v, agentID: %d, connectorID: %d , version : %s", err, draftAgent.AgentID, connectorID, version)
				lock.Lock()
	...
	if err != nil {
		logs.WarnContextf(ctx, "publish project event failed, agentID: %d, err : %v", draftAgent.AgentID, err)
	}

	uid := ctxutil.MustGetUIDFromCtx(ctx)
	if _, err := CreateShortLink(ctx, s.appContext, draftAgent.AgentID, uid, req.GetSpaceID()); err != nil {
		logs.WarnContextf(ctx, "create short link failed, agentID: %d, err: %v", draftAgent.AgentID, err)
	}
...
func publishShortcutCommand(ctx context.Context, appContext *ServiceComponents, publishInfo *entity.SingleAgentPublish, agent *entity.SingleAgent) (*entity.SingleAgent, error) {
	logs.InfoContextf(ctx, "publishShortcutCommand agentID: %d, shortcutCommand: %v", agent.AgentID, agent.ShortcutCommand)
