...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/api/model/app/bot_open_api"
	...
	"github.com/insmtx/corekg/apps/workflow/crossdomain/agent"
	...
	variableEntity "github.com/insmtx/corekg/apps/workflow/domain/memory/variables/entity"
	...
	shortcutEntity "github.com/insmtx/corekg/apps/workflow/domain/shortcutcmd/entity"
...
func (s *SingleAgentApplicationService) DeleteAgentDraft(ctx context.Context, req *developer_api.DeleteDraftBotRequest) (*developer_api.DeleteDraftBotResponse, error) {
	...
	if err != nil {
		logs.WarnContextf(ctx, "publish delete project event failed id = %v , err = %v", req.GetBotID(), err)
	}
...
func (s *SingleAgentApplicationService) ValidateAgentDraftAccess(ctx context.Context, agentID int64) (*entity.SingleAgent, error) {
	...
	allowedIDs, err := requestyygu.FilterCoreKGResourceIDsByScopePermission(ctx, *uid, int64(permission.ResourceTypeAgent), []int64{do.AgentID}, string(permission.ActionRead))
	if err != nil {
		logs.ErrorContextf(ctx, "check agent draft permission failed: user=%d agent=%d err=%v", *uid, do.AgentID, err)
		return do, err
	}
	if _, ok := allowedIDs[do.AgentID]; !ok {
		logs.ErrorContextf(ctx, "user(%d) has no permission for agent draft(%d)", *uid, do.AgentID)
...
func (s *SingleAgentApplicationService) ReportUserBehavior(ctx context.Context, req *playground.ReportUserBehaviorRequest) (resp *playground.ReportUserBehaviorResponse, err error) {
	...
	if err != nil {
		logs.WarnContextf(ctx, "publish updated project event failed id=%v, err=%v", req.ResourceID, err)
	}
...
func (s *SingleAgentApplicationService) getAgentInfo(ctx context.Context, botID int64, connectorID int64, uid int64, version *string) (*bot_common.OpenAPIBotInfo, error) {
	...
	if agentInfo == nil {
		logs.ErrorContextf(ctx, "agent(%d) is not exist", botID)
