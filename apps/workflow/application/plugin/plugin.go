...
import (
	...
	productCommon "github.com/insmtx/corekg/apps/workflow/api/model/marketplace/product_common"
	productAPI "github.com/insmtx/corekg/apps/workflow/api/model/marketplace/product_public_api"
	...
	"github.com/insmtx/corekg/apps/workflow/crossdomain/plugin/consts"
	"github.com/insmtx/corekg/apps/workflow/crossdomain/plugin/convert/api"
	"github.com/insmtx/corekg/apps/workflow/domain/permission"
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/dto"
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/entity"
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/repository"
...
func (p *PluginApplicationService) buildProductMetaInfo(ctx context.Context, plugin *entity.PluginInfo) (*productAPI.ProductMetaInfo, error) {
	...
	if err != nil {
		logs.WarnContextf(ctx, "get icon url failed with '%s', err=%v", plugin.GetIconURI(), err)
	}
...
func (p *PluginApplicationService) buildPluginProductExtraInfo(ctx context.Context, plugin *entity.PluginInfo, tools []*entity.ToolInfo) (*productAPI.PluginExtraInfo, error) {
	...
	if authInfo != nil {
		if authInfo.Type == consts.AuthzTypeOfService || authInfo.Type == consts.AuthzTypeOfOAuth {
			...
			if err != nil {
				logs.WarnContextf(ctx, "validate plugin manifest failed, err=%v", err)
			} else {
...
func (p *PluginApplicationService) validateDraftPluginAccess(ctx context.Context, pluginID int64) (plugin *entity.PluginInfo, err error) {
	...
	allowedIDs, err := requestyygu.FilterCoreKGResourceIDsByScopePermission(
		ctx,
		*uid,
		int64(permission.ResourceTypePlugin),
		[]int64{plugin.ID},
		string(permission.ActionRead),
	)
	if err != nil {
		logs.ErrorContextf(ctx, "check draft plugin permission failed: user=%d plugin=%d err=%v", *uid, plugin.ID, err)
		return plugin, err
	}
	if _, ok := allowedIDs[plugin.ID]; !ok {
		logs.ErrorContextf(ctx, "user(%d) has no permission for draft plugin(%d)", *uid, plugin.ID)
		return plugin, errorx.New(errno.ErrPluginPermissionCode, errorx.KV(errno.PluginMsgKey, "you are not the plugin owner"))
	}
...
func (p *PluginApplicationService) convertPluginsToProductInfos(ctx context.Context, plugins []*entity.PluginInfo, tools map[int64][]*entity.ToolInfo) []*productAPI.ProductInfo {
	...
	for _, plugin := range plugins {
		...
		if tool, exist := tools[plugin.ID]; exist {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "buildPluginProductExtraInfo failed: %v", err)
			} else {
				pExtra = pluginExtra
			}
		}
...
func (p *PluginApplicationService) GetCozeSaasPluginList(ctx context.Context, req *productAPI.GetProductListRequest) (resp *productAPI.GetProductListResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "ListSaasPluginProducts failed: %v", err)
		return nil, err
	}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "BatchGetSaasPluginToolsInfo failed: %v", err)
		return nil, err
	}
...
func (p *PluginApplicationService) PublicSearchSuggest(ctx context.Context, req *productAPI.SearchSuggestRequest) (resp *productAPI.SearchSuggestResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "ListSaasPluginProducts for suggestions failed: %v", err)
		return nil, err
	}
...
func (p *PluginApplicationService) GetSaasProductCategoryList(ctx context.Context, req *productAPI.GetProductCategoryListRequest) (resp *productAPI.GetProductCategoryListResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "ListSaasPluginCategories failed: %v", err)
		return nil, err
	}
...
func (p *PluginApplicationService) GetProductCallInfo(ctx context.Context, req *productAPI.GetProductCallInfoRequest) (resp *productAPI.GetProductCallInfoResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "GetSaasUserInfo failed: %v", err)
		return nil, err
	}

	benefit, err := p.userSVC.GetUserBenefit(ctx)
	if err != nil {
		logs.ErrorContextf(ctx, "GetUserBenefit failed: %v", err)
		return nil, err
	}
...
func (p *PluginApplicationService) GetMarketPluginConfig(ctx context.Context, req *productAPI.GetMarketPluginConfigRequest) (resp *productAPI.GetMarketPluginConfigResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "GetBaseConfig failed: %v", err)
		return nil, err
	}
