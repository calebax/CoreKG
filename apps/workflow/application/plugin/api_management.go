...
import (
	...
	resCommon "github.com/insmtx/corekg/apps/workflow/api/model/resource/common"
	...
	"github.com/insmtx/corekg/apps/workflow/crossdomain/plugin/convert"
...
func (p *PluginApplicationService) UpdateAPI(ctx context.Context, req *pluginAPI.UpdateAPIRequest) (resp *pluginAPI.UpdateAPIResponse, err error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "publish resource '%d' failed, err=%v", req.PluginID, err)
	}
...
func (p *PluginApplicationService) GetUpdatedAPIs(ctx context.Context, req *pluginAPI.GetUpdatedAPIsRequest) (resp *pluginAPI.GetUpdatedAPIsResponse, err error) {
	...
	for name, ot := range onlineMap {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "marshal online tool operation failed, toolID=%d, err=%v", ot.ID, err)
		...
		if err != nil {
			logs.ErrorContextf(ctx, "marshal draft tool operation failed, toolID=%d, err=%v", ot.ID, err)
...
func (p *PluginApplicationService) DebugAPI(ctx context.Context, req *pluginAPI.DebugAPIRequest) (resp *pluginAPI.DebugAPIResponse, err error) {
	...
	if err != nil {
		...
		logs.ErrorContextf(ctx, "ExecuteTool failed, err=%v", err)
	...
	if err != nil {
		logs.ErrorContextf(ctx, "ToRespAPIParameter failed, err=%v", err)
		resp.Success = false
		resp.Reason = defaultErrReason
	} else {
