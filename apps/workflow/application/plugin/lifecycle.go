...
func (p *PluginApplicationService) GetDevPluginList(ctx context.Context, req *pluginAPI.GetDevPluginListRequest) (resp *pluginAPI.GetDevPluginListResponse, err error) {
	...
	if len(res.Plugins) > 0 {
		resourceIDs := make([]int64, 0, len(res.Plugins))
		for _, pl := range res.Plugins {
			resourceIDs = append(resourceIDs, pl.ID)
		}

		allowedIDs, filterErr := requestyygu.FilterCoreKGResourceIDsByScopePermission(
			ctx,
			*uid,
		...
		if filterErr != nil {
			logs.ErrorContextf(ctx, "check draft plugin list permission failed: user=%d plugin_ids=%v err=%v", *uid, resourceIDs, filterErr)
			return nil, filterErr
		}

		for _, pl := range res.Plugins {
			if _, ok := allowedIDs[pl.ID]; !ok {
				logs.ErrorContextf(ctx, "user(%d) has no permission for draft plugin(%d)", *uid, pl.ID)
