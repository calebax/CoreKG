...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/api/model/file/upload"
	...
	"github.com/insmtx/corekg/apps/workflow/domain/upload/entity"
...
func (u *UploadService) UploadFileOpen(ctx context.Context, req *bot_open_api.UploadFileOpenRequest) (*bot_open_api.UploadFileOpenResponse, error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "parse multipart form data failed, err: %v", err)
		return nil, err
	}
...
func (u *UploadService) GetObjInfoBySessionKey(ctx context.Context, sessionKey string) (*GetObjInfoBySessionKey, error) {
	...
	if isImageUri(objKey) {
		...
		if isSVG(objKey) {
			...
			if err != nil {
				logs.ErrorContextf(ctx, "get svg dimensions failed, err: %v", err)
				// default val
				resp.Width = 100
				resp.Height = 100
				return &resp, nil
			...
			if err != nil {
				logs.ErrorContextf(ctx, "decode image failed, err: %v", err)
				// default val
				resp.Width = 100
				resp.Height = 100
				return &resp, nil
...
func getMinIORegion() string {
	...
	region := strings.TrimSpace(appCfg.Workflow.Storage.MinIO.Region)
	if region == "" {
		return "cn-xian"
	}
	return region
