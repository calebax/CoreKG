...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/domain/upload/service"
...
func (w *ApplicationService) OpenAPIChatFlowRun(ctx context.Context, req *workflow.ChatFlowRunRequest) (
	...
	if existed {
		...
		if err != nil {
			unErr := unbinding()
			if unErr != nil {
				logs.ErrorContextf(ctx, "unbinding failed, error: %v", unErr)
			}
			return nil, err
		}
	...
	if len(historyMessages) > 0 {
		...
		err = g.Wait()
		if err != nil {
			logs.WarnContextf(ctx, "create history message failed, err=%v", err)
		}
	}
		...
		defer func() {
			if err != nil {
				...
				if intermediateMessage != nil {
					...
					if mErr != nil {
						logs.WarnContextf(ctx, "create message faield, err: %v", err)
					}
				}
			}
