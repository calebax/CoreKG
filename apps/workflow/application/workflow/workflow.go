...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/api/model/data/database/table"
	"github.com/insmtx/corekg/apps/workflow/api/model/playground"
	pluginAPI "github.com/insmtx/corekg/apps/workflow/api/model/plugin_develop"
	common "github.com/insmtx/corekg/apps/workflow/api/model/plugin_develop/common"
	resource "github.com/insmtx/corekg/apps/workflow/api/model/resource/common"
	"github.com/insmtx/corekg/apps/workflow/api/model/workflow"
	"github.com/insmtx/corekg/apps/workflow/application/base/ctxutil"
	appknowledge "github.com/insmtx/corekg/apps/workflow/application/knowledge"
	appmemory "github.com/insmtx/corekg/apps/workflow/application/memory"
	appplugin "github.com/insmtx/corekg/apps/workflow/application/plugin"
	"github.com/insmtx/corekg/apps/workflow/application/user"
	"github.com/insmtx/corekg/apps/workflow/bizpkg/debugutil"
	crossknowledge "github.com/insmtx/corekg/apps/workflow/crossdomain/knowledge"
	model "github.com/insmtx/corekg/apps/workflow/crossdomain/knowledge/model"
	crosspermission "github.com/insmtx/corekg/apps/workflow/crossdomain/permission"
	pluginConsts "github.com/insmtx/corekg/apps/workflow/crossdomain/plugin/consts"
	...
	"github.com/insmtx/corekg/apps/workflow/domain/plugin/dto"
	search "github.com/insmtx/corekg/apps/workflow/domain/search/entity"
	domainWorkflow "github.com/insmtx/corekg/apps/workflow/domain/workflow"
	...
	"github.com/insmtx/corekg/apps/workflow/domain/workflow/plugin"
	...
	"github.com/insmtx/corekg/apps/workflow/pkg/i18n"
	"github.com/insmtx/corekg/apps/workflow/pkg/lang/conv"
	"github.com/insmtx/corekg/apps/workflow/pkg/lang/maps"
	...
	"github.com/insmtx/corekg/apps/workflow/pkg/safego"
		...
		for {
			...
			case <-ticker.C:
				if err := w.refreshNodeIconURLCache(context.Background()); err != nil {
					logs.ErrorContextf(ctx,"failed to refresh node icon url cache: %v", err)
				}
			}
		}
	})
...
func (w *ApplicationService) refreshNodeIconURLCache(ctx context.Context) error {
	...
	if err != nil {
		logs.ErrorContextf(ctx,"failed to list node meta for icon url cache: %v", err)
		return err
	}
		...
		for _, nodeMeta := range nodeMetaList {
			...
			eg.Go(func() error {
				if len(nodeMeta.IconURI) == 0 {
					...
					logs.WarnContextf(ctx,"node '%s' has an empty IconURI, it will have no icon", nodeMeta.Name)
					return nil
				}
				url, err := w.TosClient.GetObjectUrl(gCtx, nodeMeta.IconURI, storage.WithExpire(int64(time.Hour.Seconds())))
				if err != nil {
					logs.WarnContextf(ctx,"failed to get object url for node %s: %v", nodeMeta.Name, err)
					return err
				}
	...
	logs.InfoContextf(ctx,"node icon url cache refreshed with %d entries", len(nodeIconURLCache))
	return nil
}
...
func (w *ApplicationService) GetNodeTemplateList(ctx context.Context, req *workflow.NodeTemplateListRequest) (
	...
	for _, t := range req.NodeTypes {
		...
		if len(entityType) == 0 {
			logs.WarnContextf(ctx,"get node type %v failed, err:=%v", t, err)
			continue
		}
...
func (w *ApplicationService) UpdateWorkflowMeta(ctx context.Context, req *workflow.UpdateWorkflowMetaRequest) (
	...
	safego.Go(ctx, func() {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "publish update workflow resource failed, workflowID: %d, err: %v", workflowID, err)
		}
	})
	...
	safego.Go(ctx, func() {
		for _, id := range ids {
			if err = PublishWorkflowResource(ctx, id, nil, search.Deleted, &search.ResourceDocument{}); err != nil {
				logs.ErrorContextf(ctx, "publish delete workflow event resource failed, workflowID: %d, err: %v", id, err)
			}
		}
	})
...
func (w *ApplicationService) copyWorkflowFromAppToLibrary(ctx context.Context, workflowID int64, appID int64, related vo.ExternalResourceRelated) (map[int64]entity.IDVersionPair, []*vo.ValidateIssue, error) {
	...
	for index := range resp.CopiedWorkflows {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "failed to publish workflow resource, workflow id=%d, err=%v", wf.ID, err)
			return nil, nil, err
		}
	}
...
func (w *ApplicationService) DuplicateWorkflowsByAppID(ctx context.Context, sourceAppID, targetAppID int64, externalResource ExternalResource) (err error) {
	...
	logs.InfoContextf(ctx, "[DuplicateWorkflowsByAppID] %s", conv.DebugJsonToStr(copiedWorkflowArray))
...
func (w *ApplicationService) copyWorkflow(ctx context.Context, workflowID int64, policy vo.CopyWorkflowPolicy) (*entity.Workflow, error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "public copy workflow event failed, workflowID=%d, err=%v", wf.ID, err)
		return nil, err
	}
...
func (w *ApplicationService) publishWorkflowResource(ctx context.Context, policy *vo.PublishPolicy) error {
	...
	safego.Go(ctx, func() {
		...
		if err := PublishWorkflowResource(ctx, policy.ID, nil, search.Updated, &search.ResourceDocument{
			...
			logs.ErrorContextf(ctx, "publish workflow resource failed workflowID = %d, err: %v", policy.ID, err)
		}
	})
...
func (w *ApplicationService) CreateChatFlowRole(ctx context.Context, req *workflow.CreateChatFlowRoleRequest) (
	...
	if !IsChatFlow(wf) {
		logs.WarnContextf(ctx, "CreateChatFlowRole not chat flow, workflowID: %d", wf.ID)
...
func (w *ApplicationService) GetChatFlowRole(ctx context.Context, req *workflow.GetChatFlowRoleRequest) (
	...
	if !IsChatFlow(wf) {
		logs.WarnContextf(ctx, "GetChatFlowRole not chat flow, workflowID: %d", wf.ID)
		return nil, vo.WrapError(errno.ErrChatFlowRoleOperationFail, fmt.Errorf("workflow %d is not a chat flow", wf.ID))
	}
	...
	if role == nil {
		logs.WarnContextf(ctx, "GetChatFlowRole role nil, workflowID: %d", wf.ID)
...
func (w *ApplicationService) convertChatFlowRole(ctx context.Context, role *entity.ChatFlowRole) (*workflow.ChatFlowRole, error) {
	...
	if role.AudioConfig != "" {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "GetChatFlowRole AudioConfig UnmarshalString err: %+v", err)
	...
	if role.OnboardingInfo != "" {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "GetChatFlowRole OnboardingInfo UnmarshalString err: %+v", err)
			return nil, vo.WrapError(errno.ErrSerializationDeserializationFail, err)
		}
	}
	...
	if role.SuggestReplyInfo != "" {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "GetChatFlowRole SuggestReplyInfo UnmarshalString err: %+v", err)
			return nil, vo.WrapError(errno.ErrSerializationDeserializationFail, err)
		}
	}
	...
	if role.UserInputConfig != "" {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "GetChatFlowRole UserInputConfig UnmarshalString err: %+v", err)
			return nil, vo.WrapError(errno.ErrSerializationDeserializationFail, err)
		}
	}
	...
	if role.BackgroundImageInfo != "" {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "GetChatFlowRole BackgroundImageInfo UnmarshalString err: %+v", err)
			return nil, vo.WrapError(errno.ErrSerializationDeserializationFail, err)
		}
		if res.BackgroundImageInfo != nil {
			if res.BackgroundImageInfo.WebBackgroundImage != nil && res.BackgroundImageInfo.WebBackgroundImage.OriginImageUri != nil {
				...
				if err != nil {
					logs.ErrorContextf(ctx, "get url by uri err, err:%s", err.Error())
					return nil, err
				}
