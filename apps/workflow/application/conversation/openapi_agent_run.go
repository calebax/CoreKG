...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/api/model/conversation/message"
	"github.com/insmtx/corekg/apps/workflow/api/model/conversation/run"
	...
	agentrun "github.com/insmtx/corekg/apps/workflow/crossdomain/agentrun/model"
	crossmessage "github.com/insmtx/corekg/apps/workflow/crossdomain/message/model"
	saEntity "github.com/insmtx/corekg/apps/workflow/domain/agent/singleagent/entity"
	"github.com/insmtx/corekg/apps/workflow/domain/conversation/agentrun/entity"
	convEntity "github.com/insmtx/corekg/apps/workflow/domain/conversation/conversation/entity"
	cmdEntity "github.com/insmtx/corekg/apps/workflow/domain/shortcutcmd/entity"
	uploadService "github.com/insmtx/corekg/apps/workflow/domain/upload/service"
	sseImpl "github.com/insmtx/corekg/apps/workflow/infra/sse/impl/sse"
...
func (a *OpenapiAgentRunApplication) OpenapiAgentRun(ctx context.Context, sseSender *sseImpl.SSenderImpl, ar *run.ChatV3Request) error {
	...
	if caErr != nil {
		logs.ErrorContextf(ctx, "checkAgent err:%v", caErr)
		return caErr
	}
	...
	if ccErr != nil {
		logs.ErrorContextf(ctx, "checkConversation err:%v", ccErr)
		return ccErr
	}
	...
	if err != nil {
		logs.ErrorContextf(ctx, "buildAgentRunRequest err:%v", err)
		return err
	}
...
func (a *OpenapiAgentRunApplication) parseAdditionalMessages(ctx context.Context, ar *run.ChatV3Request) ([]*entity.AdditionalMessage, error) {
	...
	for _, item := range ar.AdditionalMessages {
		...
		if item.ContentType == run.ContentTypeMixApi {
			...
			logs.InfoContextf(ctx, "inputs:%v, err:%v", conv.DebugJsonToStr(inputs), err)
			if err != nil {
				continue
			}
...
func (a *OpenapiAgentRunApplication) pullStream(ctx context.Context, sseSender *sseImpl.SSenderImpl, streamer *schema.StreamReader[*entity.AgentRunResponse]) {
	for {
		...
		logs.InfoContextf(ctx, "chunk :%v, err:%v", conv.DebugJsonToStr(chunk), recvErr)
		if recvErr != nil {
		...
		default:
			logs.ErrorContextf(ctx, "unknow handler event:%v", chunk.Event)
		}
	}
}
...
func (a *OpenapiAgentRunApplication) OpenapiAgentRunSync(ctx context.Context, ar *run.ChatV3Request) (*run.RetrieveChatOpenResponse, error) {
	...
	startBackgroundProcessing := func() {
		if !backgroundProcessingStarted {
			...
			go func() {
				defer func() {
					if r := recover(); r != nil {
						logs.ErrorContextf(ctx, "background stream processing panic: %v", r)
					}
				}()
				for {
					...
					if recvErr != nil {
						if errors.Is(recvErr, io.EOF) {
							logs.InfoContextf(ctx, "background stream processing completed")
						} else {
							logs.ErrorContextf(ctx, "background stream processing error: %v", recvErr)
						}
						break
					}
				}
	...
	for {
		...
		case entity.RunEventCreated:
			...
			goto exitLoop
		default:
			logs.InfoContextf(ctx, "received event: %v, starting background processing and continuing to listen", chunk.Event)
			startBackgroundProcessing()
		}
	}
exitLoop:
