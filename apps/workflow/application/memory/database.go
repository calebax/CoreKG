...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/api/model/base"
	...
	model "github.com/insmtx/corekg/apps/workflow/crossdomain/database/model"
	...
	databaseEntity "github.com/insmtx/corekg/apps/workflow/domain/memory/database/entity"
...
func (d *DatabaseApplicationService) ValidateAccess(ctx context.Context, databaseID int64, tableType table.TableType) error {
	...
	if do.Databases[0].CreatorID != *uid {
		logs.ErrorContextf(ctx, "user(%d) is not the creator(%d) of the database(%d)", *uid, do.Databases[0].CreatorID, databaseID)
