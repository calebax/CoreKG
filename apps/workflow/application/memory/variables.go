...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/api/model/data/variable/kvmemory"
	...
	model "github.com/insmtx/corekg/apps/workflow/crossdomain/variables/model"
	"github.com/insmtx/corekg/apps/workflow/domain/memory/variables/entity"
...
func (v *VariableApplicationService) UpdateProjectVariable(ctx context.Context, req project_memory.UpdateProjectVariableReq) (*project_memory.UpdateProjectVariableResp, error) {
	...
	for _, v := range req.VariableList {
		if v.Channel == project_memory.VariableChannel_System &&
			...
			logs.InfoContextf(ctx, "sys variable not found, keyword: %s", v.Keyword)
			continue
		}
