...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/bizpkg/config/modelmgr/internal/model"
	"github.com/insmtx/corekg/apps/workflow/bizpkg/config/modelmgr/internal/query"
...
func (c *ModelConfig) toModel(ctx context.Context, q *model.ModelInstance) *Model {
	...
	if err != nil {
		logs.WarnContextf(ctx, "decrypt model connection failed, err: %v", err)
	}

	extra := &ModelExtra{}
	if err := json.Unmarshal([]byte(q.Extra), extra); err != nil {
		logs.WarnContextf(ctx, "unmarshal model extra (%s) failed, err: %v", q.Extra, err)
	}
