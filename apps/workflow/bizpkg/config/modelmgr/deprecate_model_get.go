...
func initOldModelConf(ctx context.Context, oss storage.Storage, c *ModelConfig) error {
	...
	for _, q := range oldModels {
		if q.Provider.IconURI != "" {
			...
			if err != nil {
				logs.WarnContextf(ctx, "get model icon url failed, err: %v", err)
			} else {
	...
	for _, old := range oldModels {
		if old.ID <= 0 {
			logs.WarnContextf(ctx, "model id is invalid, model: %v", old.ID)
			continue
		}

		_, err := c.getModelByID(ctx, old.ID)
		if err == nil {
			logs.InfoContextf(ctx, "model id %d - %s already exists", old.ID, old.DisplayInfo.Name)
			continue
		}
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			logs.InfoContextf(ctx, "sync old model id %d - %s to db success, new id: %d ", old.ID, old.DisplayInfo.Name, id)
			continue
		}
...
func (c *ModelConfig) UseOldModelConf(ctx context.Context) (bool, error) {
	...
	if err != nil {
		if errors.Is(err, kvstore.ErrKeyNotFound) {
			logs.InfoContextf(ctx, "[UseOldModelConf] will use old model")
...
func (c *ModelConfig) SetDoNotUseOldModelConf(ctx context.Context) error {
	...
	if err != nil {
		logs.WarnContextf(ctx, "set use new model list failed, err: %v , will try to set use new model flag", err)
	}
