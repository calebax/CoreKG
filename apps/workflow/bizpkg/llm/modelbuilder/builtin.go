...
func GetBuiltinChatModel(ctx context.Context, envPrefix string) (bcm BaseChatModel, configured bool, err error) {
	...
	if ok {
		logs.DebugContextf(ctx, "builtin chat model in context: %v", bcm)
		return bcm, true, nil
	}
	...
	if err == nil {
		...
		logs.WarnContextf(ctx, "GetBuiltinChatModelConfig failed : %v", err)
	}
	...
	for _, m := range modelList {
		...
		if err != nil {
			logs.WarnContextf(ctx, "build model %v %v failed: %v", m.Provider.Name, m.Provider.ModelClass.String(), err)
			continue
		}

		if err = checkModelConfig(ctx, bcm); err == nil {
			logs.DebugContextf(ctx, "build model %v %v success, in model list", m.Provider.Name, m.Provider.ModelClass.String())
...
func checkModelConfig(ctx context.Context, bcm BaseChatModel) (err error) {
	...
	if err != nil {
		logs.WarnContextf(ctx, "builtin chat model not configured: %v", err)
		return fmt.Errorf("builtin chat model not configured: %w", err)
	}

	logs.DebugContextf(ctx, "generate model respMsgs: %v", respMsgs)
