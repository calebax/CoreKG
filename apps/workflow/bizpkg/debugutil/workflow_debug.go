...
func GetWorkflowDebugURL(ctx context.Context, workflowID, spaceID, executeID int64) string {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "[GetWorkflowDebugURL] get base config failed, use default debug url instead, err: %v", err)
		return defaultURL
	}

	workFlowURL, err := url.JoinPath(serverHost, "work_flow")
	if err != nil {
		logs.ErrorContextf(ctx, "[GetWorkflowDebugURL] join path failed, use default debug url instead, err: %v", err)
		return defaultURL
	}

	u, err := url.Parse(workFlowURL)
	if err != nil {
		logs.ErrorContextf(ctx, "[GetWorkflowDebugURL] parse workflow url failed, use default debug url instead, err: %v", err)
		return defaultURL
	}
