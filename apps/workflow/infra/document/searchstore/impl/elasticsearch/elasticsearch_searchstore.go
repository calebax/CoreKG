...
import (
	...
	esembedding "github.com/insmtx/corekg/apps/workflow/infra/embedding"
	"github.com/insmtx/corekg/apps/workflow/infra/es"
	"github.com/insmtx/corekg/apps/workflow/pkg/lang/ptr"
)

const denseVectorFieldPrefix = "dense_vector_"

func denseVectorFieldName(field string) string {
	return denseVectorFieldPrefix + field
}
...
type esSearchStore struct {
	...
	indexName  string
	vectorMode bool
	embedding  esembedding.Embedder
}
...
func (e *esSearchStore) Store(ctx context.Context, docs []*schema.Document, opts ...indexer.Option) (ids []string, err error) {
	...
	if e.vectorMode && e.embedding != nil {
		texts := make([]string, 0, len(docs))
		for _, doc := range docs {
			texts = append(texts, doc.Content)
		}

		vectors, err := e.embedding.EmbedStrings(ctx, texts)
		if err != nil {
			return nil, fmt.Errorf("[Store] embedding failed: %w", err)
		}
		...
		for i, doc := range docs {
			...
			if i < len(vectors) {
				fieldMapping[denseVectorFieldName(searchstore.FieldTextContent)] = vectors[i]
			}
...
func (e *esSearchStore) Retrieve(ctx context.Context, query string, opts ...retriever.Option) ([]*schema.Document, error) {
	if e.vectorMode && e.embedding != nil {
		return e.vectorRetrieve(ctx, query, opts...)
	}
	return e.textRetrieve(ctx, query, opts...)
}

func (e *esSearchStore) vectorRetrieve(ctx context.Context, query string, opts ...retriever.Option) ([]*schema.Document, error) {
	options := retriever.GetCommonOptions(&retriever.Options{TopK: ptr.Of(topK)}, opts...)

	vectors, err := e.embedding.EmbedStrings(ctx, []string{query})
	if err != nil {
		return nil, fmt.Errorf("[Retrieve] embedding failed: %w", err)
	}
	if len(vectors) == 0 {
		return nil, fmt.Errorf("[Retrieve] embedding returned empty vectors")
	}
	...
	req := &es.Request{
		Knn: &es.KnnQuery{
			Field:         denseVectorFieldName(searchstore.FieldTextContent),
	...
	if dsl != nil || implSpecOptions.MultiMatch != nil {
		req.Query = &es.Query{Bool: &es.BoolQuery{}}
	...
	resp, err := e.config.Client.Search(ctx, e.indexName, req)
	if err != nil {
		return nil, err
	}

	return e.parseSearchResult(resp)
}

func (e *esSearchStore) textRetrieve(ctx context.Context, query string, opts ...retriever.Option) ([]*schema.Document, error) {
	var (
		cli   = e.config.Client
		index = e.indexName
...
func (e *esSearchStore) parseSearchResult(resp *es.Response) (docs []*schema.Document, err error) {
	...
	for i, hit := range resp.Hits.Hits {
		...
		for field, val := range src {
			if len(field) > len(denseVectorFieldPrefix) && field[:len(denseVectorFieldPrefix)] == denseVectorFieldPrefix {
				continue
			}
			ok := true
			switch field {
