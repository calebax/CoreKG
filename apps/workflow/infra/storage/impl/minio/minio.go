...
const defaultMinIORegion = "cn-xian"
...
func getMinioClient(ctx context.Context, endpoint, accessKeyID, secretAccessKey, bucketName, region string, useSSL bool) (*minioClient, error) {
	...
	err = m.createBucketIfNeed(ctx, client, bucketName, region)
	if err != nil {
		return nil, fmt.Errorf("init minio client failed %v", err)
	}
...
func (m *minioClient) test() {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "upload file failed: %v", err)
	}

	f, err := m.HeadObject(ctx, objectName, storage.WithGetTagging(true), storage.WithURL(true))
	if err != nil {
		logs.ErrorContextf(ctx, "head object failed: %v", err)
	}
	if f != nil {
		logs.InfoContextf(ctx, "head object success, f: %v, tagging: %v", *f, f.Tagging)
	}

	f, err = m.HeadObject(ctx, "not_exit.txt", storage.WithGetTagging(true))
	logs.InfoContextf(context.Background(), "HeadObject not exit success, f: %v, err: %v", f, err)

	logs.InfoContextf(ctx, "upload file success")

	files, err := m.ListAllObjects(ctx, "test-file-", storage.WithGetTagging(true), storage.WithURL(true))
	if err != nil {
		logs.ErrorContextf(ctx, "list objects failed: %v", err)
	}

	logs.InfoContextf(ctx, "list objects success, files.len: %v", len(files))

	url, err := m.GetObjectUrl(ctx, objectName)
	if err != nil {
		logs.ErrorContextf(ctx, "get file url failed: %v", err)
	}

	logs.InfoContextf(ctx, "get file url success, url: %s", url)

	content, err := m.GetObject(ctx, objectName)
	if err != nil {
		logs.ErrorContextf(ctx, "download file failed: %v", err)
	}

	logs.InfoContextf(ctx, "download file success, content: %s", string(content))

	err = m.DeleteObject(ctx, objectName)
	if err != nil {
		logs.ErrorContextf(ctx, "delete object failed: %v", err)
	}

	logs.InfoContextf(ctx, "delete object success")
}
...
func (m *minioClient) ListAllObjects(ctx context.Context, prefix string, opts ...storage.GetOptFn) ([]*storage.FileInfo, error) {
	...
	for object := range objectCh {
		...
		logs.DebugContextf(ctx, "key = %s, lastModified = %s, eTag = %s, size = %d, tagging = %v",
