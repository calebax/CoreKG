...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/infra/storage/impl/internal/fileutil"
	"github.com/insmtx/corekg/apps/workflow/pkg/goutil"
...
func (t *tosClient) test() {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "PutObject failed, objectKey: %s, err: %v", objectKey, err)
	}

	f, err := t.HeadObject(ctx, objectKey, storage.WithGetTagging(true), storage.WithURL(true))
	if err != nil {
		logs.ErrorContextf(ctx, "HeadObject failed, objectKey: %s, err: %v", objectKey, err)
	}
	logs.InfoContextf(ctx, "HeadObject file success, f: %v, err: %v", conv.DebugJsonToStr(f), err)

	if f != nil {
		logs.InfoContextf(ctx, "HeadObject success, f: %v, tagging: %v", *f, f.Tagging)
	}

	f, err = t.HeadObject(ctx, "not_exit.txt", storage.WithGetTagging(true), storage.WithURL(true))
	logs.InfoContextf(ctx, "HeadObject not exit success, f: %v, err: %v", f, err)
	...
	if err != nil {
		logs.ErrorContextf(ctx, "GetObject failed, objectKey: %s, err: %v", objectKey, err)
	}

	logs.InfoContextf(ctx, "GetObject content: %s", string(content))

	// Test Get URL
	url, err := t.GetObjectUrl(ctx, objectKey)
	if err != nil {
		logs.ErrorContextf(ctx, "GetObjectUrl failed, objectKey: %s, err: %v", objectKey, err)
	}

	logs.InfoContextf(ctx, "GetObjectUrl url: %s", url)

	// test delete
	err = t.DeleteObject(ctx, objectKey)
	if err != nil {
		logs.ErrorContextf(ctx, "DeleteObject failed, objectKey: %s, err: %v", objectKey, err)
	}
}
...
func (t *tosClient) CheckAndCreateBucket(ctx context.Context) error {
	...
	if serverErr.StatusCode == http.StatusNotFound {
		...
		logs.InfoContextf(ctx, "Bucket not found.")
		resp, err := client.CreateBucketV2(ctx, &tos.CreateBucketV2Input{
			Bucket: bucketName,
			ACL:    enum.ACLPrivate,
		})

		logs.InfoContextf(ctx, "Bucket Create resp: %v, err: %v", conv.DebugJsonToStr(resp), err)
		return err
	}
...
func (t *tosClient) GetObject(ctx context.Context, objectKey string) ([]byte, error) {
	...
	// logs.DebugContextf(ctx, "GetObject resp: %v, err: %v", conv.DebugJsonToStr(getOutput), err)
...
func (t *tosClient) ListObjectsPaginated(ctx context.Context, input *storage.ListObjectsPaginatedInput, opts ...storage.GetOptFn) (*storage.ListObjectsPaginatedOutput, error) {
	...
	for _, obj := range output.Contents {
		if obj.Size == 0 && strings.HasSuffix(obj.Key, "/") {
			logs.DebugContextf(ctx, "[ListObjectsPaginated] skip dir: %s", obj.Key)
			continue
		}
...
func (t *tosClient) ListAllObjects(ctx context.Context, prefix string, opts ...storage.GetOptFn) ([]*storage.FileInfo, error) {
	...
	for {
		...
		for _, object := range output.Files {
			logs.DebugContextf(ctx, "key = %s, lastModified = %s, eTag = %s, size = %d, tagging = %v, url = %s",
				object.Key, object.LastModified, object.ETag, object.Size, object.Tagging, object.URL)
			files = append(files, object)
		}

		cursor = output.Cursor
		logs.DebugContextf(ctx, "IsTruncated = %v, Cursor = %s", output.IsTruncated, output.Cursor)

		if len(files) >= MaxListObjects {
			logs.ErrorContextf(ctx, "[ListObjects] max list objects reached, total: %d", len(files))
			break
		}
