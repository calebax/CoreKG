...
func (h *MessageHandler) HandleMessage(m *nsq.Message) error {
	...
	msgCtx := logs.WithContextFields(context.Background(), "reqid", traceID)

	logs.DebugContextf(msgCtx, "[Subscribe] receive msg : %v \n", conv.DebugJsonToStr(msg))
	err := h.ConsumerHandler.HandleMessage(msgCtx, msg)
	if err != nil {
		logs.ErrorContextf(msgCtx, "[Subscribe] handle msg failed, topic : %s , group : %s, err: %v \n", msg.Topic, msg.Group, err)
		return err
	}

	logs.DebugContextf(msgCtx, "subscribe callback: %v \n", conv.DebugJsonToStr(msg))
