...
func RegisterConsumer(serviceURL, topic, group string, consumerHandler eventbus.ConsumerHandler, opts ...eventbus.ConsumerOpt) error {
	...
	if jwtToken := conf.GetAppConfig().Workflow.MQ.Pulsar.JWTToken; jwtToken != "" {
		...
		for {
			...
			default:
				...
				if err := consumerHandler.HandleMessage(msgCtx, eventMsg); err != nil {
					logs.ErrorContextf(msgCtx, "handle pulsar message failed, topic: %s, group: %s, err: %v", topic, group, err)
