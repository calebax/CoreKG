...
import (
	...
	"github.com/insmtx/corekg/apps/workflow/pkg/lang/signal"
...
func RegisterConsumer(serverURL, topic, group string, consumerHandler eventbus.ConsumerHandler, opts ...eventbus.ConsumerOpt) error {
	...
	natsConf := conf.GetAppConfig().Workflow.MQ.NATS
	useJetStream := natsConf.UseJetStream
...
func startJetStreamConsumer(ctx context.Context, nc *nats.Conn, topic, group string, consumerHandler eventbus.ConsumerHandler) error {
	...
	safego.Go(ctx, func() {
		...
		if err != nil {
			logs.ErrorContextf(ctx, "create NATS JetStream subscription failed: %v", err)
			return
		}
		...
		for {
			select {
			case <-ctx.Done():
				logs.InfoContextf(ctx, "NATS JetStream consumer stopped for topic: %s, group: %s", topic, group)
				return
			default:
				...
				if err != nil {
					...
					logs.ErrorContextf(ctx, "fetch NATS JetStream message error: %v", err)
					continue
				}
				...
				if len(msgs) > 0 {
					...
					// Inject trace ID for this message
					traceID := uuid.New().String()
					msgCtx := logs.WithContextFields(ctx, "reqid", traceID)

					// Handle message with context
					if err := consumerHandler.HandleMessage(msgCtx, eventMsg); err != nil {
						logs.ErrorContextf(msgCtx, "handle NATS JetStream message failed, topic: %s, group: %s, err: %v", topic, group, err)
...
func startCoreConsumer(ctx context.Context, nc *nats.Conn, topic, group string, consumerHandler eventbus.ConsumerHandler) error {
	...
	safego.Go(ctx, func() {
		...
		sub, err := nc.QueueSubscribe(topic, group, func(msg *nats.Msg) {
			...
			// Handle message with context
			if err := consumerHandler.HandleMessage(msgCtx, eventMsg); err != nil {
				logs.ErrorContextf(msgCtx, "handle NATS core message failed, topic: %s, group: %s, err: %v", topic, group, err)
				// For core NATS, we can't nack, just log the error
				return
			}

			logs.DebugContextf(msgCtx, "successfully processed NATS core message, topic: %s, group: %s", topic, group)
		})

		if err != nil {
			logs.ErrorContextf(ctx, "create NATS core subscription failed: %v", err)
			return
		}
		defer sub.Unsubscribe()
		...
		<-ctx.Done()
		logs.InfoContextf(ctx, "NATS core consumer stopped for topic: %s, group: %s", topic, group)
	})
...
func addAuthentication(options *[]nats.Option) error {
	...
	if natsConf.JWTToken != "" {
		if natsConf.NKeySeed == "" {
			return fmt.Errorf("NATS_NKEY_SEED is required when using JWT authentication")
		}
		*options = append(*options, nats.UserJWTAndSeed(natsConf.JWTToken, natsConf.NKeySeed))
		return nil
	}

	if natsConf.Username != "" {
		*options = append(*options, nats.UserInfo(natsConf.Username, natsConf.Password))
		return nil
	}

	if natsConf.Token != "" {
		*options = append(*options, nats.Token(natsConf.Token))
		return nil
	}
