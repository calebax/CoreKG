...
func getESShardCount() string {
	appCfg := conf.GetAppConfig()
	shards := appCfg.Workflow.Elasticsearch.NumberOfShards
	if shards == "" {
		shards = "1"
	}
	return shards
...
func getESReplicaCount() string {
	appCfg := conf.GetAppConfig()
	replicas := appCfg.Workflow.Elasticsearch.NumberOfReplicas
	if replicas == "" {
		replicas = "1"
	}
	return replicas
