...
import (
	...
	updateapi "github.com/elastic/go-elasticsearch/v8/typedapi/core/update"
	createapi "github.com/elastic/go-elasticsearch/v8/typedapi/indices/create"
...
func newES8() (Client, error) {
	...
	addresses, err := parseClusterEndpoints(appCfg.Workflow.Elasticsearch.Addr)
	if err != nil {
		return nil, err
	}
	esUsername := appCfg.Workflow.Elasticsearch.Username
	esPassword := appCfg.Workflow.Elasticsearch.Password
...
func (c *es8Client) Create(ctx context.Context, index, id string, document any) error {
	_, err := c.esClient.Index(index).Request(document).Id(id).Do(ctx)
	return err
}

func (c *es8Client) Update(ctx context.Context, index, id string, document any) error {
	_, err := c.esClient.Core.Update(index, id).Request(&updateapi.Request{Doc: document}).Do(ctx)
	return err
}

func (c *es8Client) Delete(ctx context.Context, index, id string) error {
	_, err := c.esClient.Core.Delete(index, id).Do(ctx)
	return err
}

func (c *es8Client) Exists(ctx context.Context, index string) (bool, error) {
	exist, err := c.esClient.Indices.Exists(index).IsSuccess(ctx)
	if err != nil {
		return false, err
	}
...
func (c *es8Client) query2ESQuery(q *Query) *types.Query {
	...
	if q.Bool.MinimumShouldMatch != nil {
		msm := types.MinimumShouldMatch(*q.Bool.MinimumShouldMatch)
		typesQ.Bool.MinimumShouldMatch = &msm
	}
...
func (c *es8Client) Search(ctx context.Context, index string, req *Request) (*Response, error) {
	...
	if req.Knn != nil {
		esReq.Knn = &types.KnnQuery{
			Field:         req.Knn.Field,
			QueryVector:   req.Knn.QueryVector,
			K:             int64(req.Knn.K),
			NumCandidates: int64(req.Knn.NumCandidates),
		}
	}
	...
	if req.From != nil {
		...
		for _, v := range req.SearchAfter {
			esReq.SearchAfter = append(esReq.SearchAfter, v)
		}
	}

	logs.DebugContextf(ctx, "Elasticsearch Request: %s\n", conv.DebugJsonToStr(esReq))
...
func (c *es8Client) CreateIndex(ctx context.Context, index string, properties map[string]any) error {
	...
	for k, v := range properties {
		p, ok := v.(types.Property)
		if ok {
			propertiesMap[k] = p
		}
	}

	if _, err := c.esClient.Indices.Create(index).Request(&createapi.Request{
		Mappings: &types.TypeMapping{
			Properties: propertiesMap,
		},
		Settings: &types.IndexSettings{
			NumberOfShards:   getESShardCount(),
			NumberOfReplicas: getESReplicaCount(),
		},
	}).Do(ctx); err != nil {
		return err
	}
...
func (c *es8Client) DeleteIndex(ctx context.Context, index string) error {
	_, err := c.esClient.Indices.Delete(index).
		IgnoreUnavailable(true).Do(ctx)
	return err
}

func (c *es8Client) NewBulkIndexer(index string) (BulkIndexer, error) {
	apiClient := &elasticsearch.Client{
		API: nil,
	}
	apiClient.BaseClient = c.esClient.BaseClient
...
func (t *es8Types) NewDenseVectorProperty(dims int64) any {
	return types.DenseVectorProperty{
		Dims:       int(dims),
		Index:      ptr.Of(true),
		Similarity: ptr.Of("cosine"),
		Type:       "dense_vector",
	}
}
