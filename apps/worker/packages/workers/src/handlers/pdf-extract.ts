...
const handler: TaskHandlerFn = async (ctx, payload) => {
  const tmpDir = join("/tmp", `yg_pdf_${Date.now()}`);
  const storageDir = join(tmpDir, "storage");
  try {
    await mkdir(storageDir, { recursive: true });
    const ext = extname(new URL(payload.file_url).pathname) || ".pdf";
    ...
    await daemonProcessPdf(ctx.daemonUrl, {
      filePath,
    ...
    const fileList = results.slice(0, 10).map((r: { key: string }) => r.key);
...
export const pdfExtractHandler = handler;
