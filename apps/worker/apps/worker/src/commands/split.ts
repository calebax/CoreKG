...
export function createSplitCommand(): Command {
  return new Command("split")
    ...
    .option("--file-ext <ext>", "File extension", "")
    .option("--algo-url <url>", "Algo service URL")
    .action(async (opts) => {
      try {
        const ctx = buildLocalContext({ algoUrl: opts.algoUrl });
        const result = await splitTextChunkHandler(ctx, {
          task_type: "split_text_chunk",
          file_id: opts.fileId,
