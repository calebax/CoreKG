...
import { analysisHandler } from "@corekg/workers";
import { buildLocalContext } from "./build-context.js";

const logger = createLogger("cmd-analysis");
...
export function createAnalysisCommand(): Command {
  return new Command("analysis")
    ...
    .option("--output <path>", "Output file path for result")
    .action(async (opts) => {
      try {
        const ctx = buildLocalContext();
        const result = await analysisHandler(ctx, {
          task_type: "analysis",
          file_id: "0",
