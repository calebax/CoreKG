...
func WithSheetName(sheetName string) parser.Option {
	return parser.WrapImplSpecificOptFn(func(o *customOptions) {
...
func (p *ExcelParser) Parse(ctx context.Context, reader io.Reader, opts ...parser.Option) ([]*schema.Document, error) {
	...
	opt := parser.GetImplSpecificOptions(&customOptions{}, opts...)

	p.f = xlsxFile
	var docs []*schema.Document
	for _, sheetName := range p.f.GetSheetList() {
		if opt.sheetName != "" {
			if opt.sheetName != sheetName {
				continue
			}
		}
		sheet := &Sheet{
		...
		if err := sheet.parseSheet(); err != nil {
			logs.ErrorContextf(ctx, "parseSheet failed, sheetName: %s, err: %v", sheetName, err)
			continue
		}
		for _, table := range sheet.TableSchemas {
			docs = append(docs, table.Docs...)
		}
	}
