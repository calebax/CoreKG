...
func ProxySocks5FromSetting(cli *http.Client, group, key string) error {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "ProxySocks5FromSetting: get proxy config failed, %s", err)
		return err
	}
	auth := &proxy.Auth{
		User:     opt.Username,
	...
	dialer, err := proxy.SOCKS5("tcp", opt.Addr, auth, proxy.Direct)
	if err != nil {
		logs.ErrorContextf(ctx, "ProxySocks5FromSetting: can't connect to the proxy: %s", err)
		return err
	}
	...
	if cli.Transport == nil {
		...
		if t, ok := cli.Transport.(*http.Transport); ok {
			...
			logs.ErrorContextf(ctx, "ProxySocks5FromSetting: unsupported transport: %T", cli.Transport)
...
func ProxyConfigFromSetting(group, key string) (*config.ProxyConfig, error) {
	...
	if err != nil {
		logs.ErrorContextf(ctx, "ProxyConfigFromSetting: get proxy config failed, %s", err)
		return nil, err
	}
	return opt, nil
}
