...
func (i UintArray) Slice() (arr []uint) {
	...
	if err != nil {
		logs.ErrorContextf(context.TODO(), "UintArray %s decode failed, %s", i, err)
		return
	}
	return
}
...
// Len 返回 UintArray 的长度
func (i *UintArray) Len() int {
	arr := i.Slice()
	return len(arr)
}

func (i *UintArray) Value(targ uint) int {
	for i, v := range i.Slice() {
		if targ == v {
			return i
		}
	}
