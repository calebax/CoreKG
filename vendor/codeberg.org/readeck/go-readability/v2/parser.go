...
func (ps *Parser) removeNodes(nodeList []*html.Node, filterFn func(*html.Node) bool) {
	for _, node := range slices.Backward(nodeList) {
...
func (ps *Parser) someNode(nodeList []*html.Node, fn func(*html.Node) bool) bool {
	return slices.ContainsFunc(nodeList, fn)
}
...
func (ps *Parser) getAllNodesWithTag(node *html.Node, tagNames ...string) []*html.Node {
	result := make([]*html.Node, 0, len(tagNames))
...
func (ps *Parser) replaceBrs(elem *html.Node) {
	replaceBr := func(br *html.Node) *html.Node {
		...
		for next != nil {
			...
			if !ps.isPhrasingContent(next, false) {
				break
			}
	...
	ps.forEachNode(dom.GetElementsByTagName(articleContent, "table"), func(table *html.Node, _ int) {
		...
		if ps.hasSingleTagInsideElement(tbody, "tr") {
			...
			if ps.hasSingleTagInsideElement(row, "td") {
				...
				newTag := "div"
				if ps.everyNode(dom.ChildNodes(cell), func(child *html.Node) bool {
					return ps.isPhrasingContent(child, false)
				}) {
					newTag = "p"
				}
...
func (ps *Parser) grabArticle() *html.Node {
	...
	grabLoop:
		for node != nil {
			...
			// elements into p's
			if nodeTagName == "div" && !ps.hasAncestorTag(node, "pre", 3, nil) {
				// Put phrasing content into paragraphs.
				var p *html.Node
				childNode := node.FirstChild
				for childNode != nil {
					nextSibling := childNode.NextSibling
					if ps.isPhrasingContent(childNode, true) {
						if p != nil {
...
func (ps *Parser) getJSONLD() map[string]string {
	...
	ps.forEachNode(scripts, func(jsonLdElement *html.Node, _ int) {
		...
		case []any:
			for _, item := range pc {
				if parsedItem, ok := item.(map[string]any); ok {
		...
		case []any:
			...
			for _, author := range val {
				objAuthor, isObj := author.(map[string]any)
				if !isObj {
					continue
				}
		...
		// Publisher
		if objPublisher, isObj := parsed["publisher"].(map[string]any); isObj {
...
func (ps *Parser) hasChildBlockElement(element *html.Node) bool {
	for child := element.FirstChild; child != nil; child = child.NextSibling {
		...
		_, exist := divToPElems[child.Data]
		if exist || ps.hasChildBlockElement(child) {
			return true
		}
	}
	return false
...
func (ps *Parser) isPhrasingContent(node *html.Node, descend bool) bool {
	switch node.Type {
	...
	case atom.A,
		...
		if descend {
			for child := node.FirstChild; child != nil; child = child.NextSibling {
				if !ps.isPhrasingContent(child, true) {
					return false
				}
			}
		}
...
func (ps *Parser) cleanStyles(node *html.Node) {
	...
	for i, v := range slices.Backward(node.Attr) {
		switch v.Key {
		case "width", "height":
