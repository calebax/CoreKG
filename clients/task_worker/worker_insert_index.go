...
func InsertIndexCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			//logs.Infof("Starting insert index graph worker with "+"worker ID: %s, task type: %s, base URL: %s, workersvr URL %s",workerID, taskType, baseURL, workerServerURL)
...
func (w *insertIndexWorker) PreRun() error {
	...
	if err = nbgraph.DeleteFilesEntAndRel(context.TODO(), w.payload.ForestID, []uint{w.payload.FileID}, w.payload.ESIndex, NewNBCli(&appCfg.Nebula)); err != nil {
		logs.ErrorContextf(w.ctx, "Failed to delete nebula entities/relation: %v", err)
		return err
	}
	// 删除es数据
	err = DeleteFileReferences(w.escli, w.payload.ESIndex, []uint{w.payload.FileID})
	if err != nil {
