...
func splitTextChunkCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			//logs.Infof("Starting Split text chunk worker with "+"worker ID: %s, task type: %s, base URL: %s, workersvr URL %s", workerID, taskType, baseURL, workerServerURL)
...
func (w *splitTextChunkWorker) PreRun() error {
	//*删除之前脏数据
	//*删除nebula数据
	if err := DeleteFiles(context.TODO(), w.payload.ForestID, []uint{w.payload.FileID}, w.payload.ESIndex, NewNBCli(&appCfg.Nebula)); err != nil {
		logs.ErrorContextf(w.ctx, "Failed to delete nebula entities/relation: %v", err)
		return err
	}
	//*删除es数据
	err := DeleteFileReferencesFileChunk(w.escli, w.payload.ESIndex, []uint{w.payload.FileID})
	if err != nil {
...
func DeleteFileReferences(cli *elasticsearch.Client, indexName string, fileIds []uint) error {
	sourceStr := `
...
func DeleteFileReferencesFileChunk(cli *elasticsearch.Client, indexName string, fileIds []uint) error {
	sourceStr := `
