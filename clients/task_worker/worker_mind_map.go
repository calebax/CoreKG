...
func init() {
	rootCmd.AddCommand(mindMapCmd())
}

func mindMapCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "mindmap",
		Short: "get file's mindmap by agent",
		Run: func(cmd *cobra.Command, args []string) {
			//logs.Infof("Starting Mind map worker with worker ID: %s, task type: %s, webserver URL: %s", workerID, taskType, workerServerURL)
...
func mindMapRoutine(ictx context.Context, idx int) {
	ctx := logs.WithContextFields(ictx, "routine_idx", idx)
	logs.InfoContextf(ctx, "Mind map worker started with routine index: %d", idx)
	for {
		select {
		...
		default:
			...
			logs.InfoContextf(ctx, "Received task ID: %d, %v", taskId, payload)
			newMindMapWorker(ctx, taskId, payload).Run()
		}
	}
}

type mindMapWorker struct {
	ctx     context.Context
	taskID  uint
	payload *ragtask.TaskPayload
	rootDir string
...
func newMindMapWorker(ctx context.Context, taskID uint, payload *ragtask.TaskPayload) *mindMapWorker {
	taskKey := fmt.Sprintf("%v-%s", taskID, encryptor.UUID())
	return &mindMapWorker{
		ctx:     ctx,
		taskID:  taskID,
		payload: payload,
		rootDir: filepath.Join(os.TempDir(), "yg_mindMap", taskKey),
...
func (w *mindMapWorker) Run() (err error) {
	defer func() {
		w.Close()
		if err != nil {
	...
	titles := strings.Join(ExtractMarkdownTitles(w.content), "\n")
	...
	logs.DebugContextf(w.ctx, "analysis agent chat response: %v", resp)
	...
	if err != nil {
		logs.ErrorContextf(w.ctx, "ProcessEmbeddedUuid content failed: %v", err)
		return err
	}
	w.r = strings.NewReader(embeddedUuid)
	//upload result file
...
func (w *mindMapWorker) PostRun() error {
	if err := w.s3Cli.Save(w.ctx,
		&storage.FileInfo{
...
func (w *mindMapWorker) Init() error {
...
func (w *mindMapWorker) Close() error {
	if debug {
		return nil
	}
...
func (w *mindMapWorker) StoragePath() string {
