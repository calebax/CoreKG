...
func videoExtraTextCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			//logs.Infof("Starting Video text extraction worker with worker ID: %s, task type: %s, base URL: %s", workerID, taskType, baseURL)
			...
			if false {
				...
				payload := &ragtask.TaskPayload{
					FileURL:    "https://example.com:53081/dev-knownow/test/a/AP安装部署指南.mp4",
					UploadPath: "test/ttt5",
					Bucket:     "dev-knownow",
