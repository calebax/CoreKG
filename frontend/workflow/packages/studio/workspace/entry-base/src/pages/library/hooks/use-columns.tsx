...
export const useGetColumns = ({
  ...
  canManageResource?: (resource: ResourceInfo) => boolean;
  ...
  return [
    ...
    // {
    //   title: I18n.t('library_source' as any, {}, 'Source'),
    //   dataIndex: 'source_type',
    //   width: responsiveTableColumn(SOURCE_COL_WIDTH, SOURCE_COL_WIDTH),
    //   render: (_v, record) => {
    ...
    //       return (
    //         <div
    //           data-testid="workspace.library.item.source"
    //           className="text-[14px] font-[400]"
    //         >
    //           {I18n.t(
    //             'library_source_coze' as any,
    //             {},
    //             'Coze Official',
    //           )}
    //         </div>
    ...
    //     let sourceText = '';
    //     if (sourceType === 'corekg') {
    //       sourceText = I18n.t(
    //         'library_source_corekg' as any,
    //         {},
    //         'CoreKG Platform Sync',
    //       );
    //     } else if (sourceType === 'coze') {
    //       sourceText = I18n.t(
    //         'library_source_coze' as any,
    //         {},
    //         'Coze Official',
    ...
    // },
    {
      ...
      render: (_v, record) => {
        ...
        // 检查用户是否有管理权限
        const hasManagePermission = canManageResource
          ? canManageResource(record)
          : true;
        ...
        if (!hasManagePermission) {
          return (
            <div
              ...
              className="text-left text-[14px] font-[400]"
            >
              -
            </div>
          );
        ...
        return (
          <div
            data-testid="workspace.library.item.actions"
            className="text-left" // 确保内容左对齐
            onClick={e => {
              stopPro(e);
            }}
          >
            {config?.renderActions(record, reloadList, onOptimisticDelete) ??
              null}
          </div>
        );
      },
