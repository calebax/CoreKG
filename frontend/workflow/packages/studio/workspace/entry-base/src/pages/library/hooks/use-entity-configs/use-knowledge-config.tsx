...
import { KnowledgeApi, PluginDevelopApi } from '@coze-arch/bot-api';
...
const renderKnowledgeItem = (item: ResourceInfo) => {
  ...
  // Determine tag color based on res_sub_type
  const getTagColor = (subType: number | undefined): 'yellow' | 'brand' => {
    if (subType === 11 || subType === 12) {
      return 'yellow'; // yellow color for multimodal and excel
    }
    return 'brand'; // Blue color for text, table, image
  };
      ...
      tag={
        <>
          ...
          {knowledgeTag ? (
            <Tag
              data-testid="workspace.library.item.tag"
              color={getTagColor(item.res_sub_type)}
              size="mini"
              className="flex-shrink-0 flex-grow-0"
            >
              {I18n.t(knowledgeTag)}
...
const getTypeFilters = () => ({
  ...
  children: [
    ...
    {
      label: (
        <span data-testid="space.library.filter.knowledge.coze_official">
          {I18n.t('library_filter_tags_coze_official')}
        </span>
      ),
      value: 0,
      children: [
    ...
    {
      label: (
        <span data-testid="space.library.filter.knowledge.coreKG_official">
          {I18n.t('library_filter_tags_corekg_official')}
        </span>
      ),
      value: 1,
      children: [
        {
          label: (
            <span data-testid="space.library.filter.knowledge.multimodal">
              {I18n.t('library_filter_tags_multimodal')}
            </span>
          ),
          value: 11,
        },
        {
         ...
         {
          label: (
            <span data-testid="space.library.filter.knowledge.qa">
              {I18n.t('library_filter_tags_qapair')}
            </span>
          ),
          value: 13,
        },
...
export const useKnowledgeConfig: UseEntityConfigHook = ({
  ...
  // Use debounce to prevent multiple toast messages
  const { run: showCoreKGToast } = useDebounceFn(
    () => {
      Toast.info(
        I18n.t('library_corekg_not_available' as I18nKeysNoOptionsType),
      );
    },
    { wait: 1000, leading: true, trailing: false }, // 1 second debounce
  );
  const {
  ...
  return {
    ...
    config: {
      ...
      onItemClick: (item: ResourceInfo) => {
        ...
        // 11是多模态知识库 12是excel知识库
        if ([11,12].includes(item.res_sub_type!)) {
          // Navigate to CoreKG text knowledge workspace with corekg_detail_id as forestId
          const forestId = item.corekg_detail_id || item.res_id;
          const resourceName = encodeURIComponent(item.name || '');
          navigate(
            `/space/${spaceId}/knowledge/${item.res_id}?from=library&corekg=true&forestId=${forestId}&name=${resourceName}`,
          );
          return;
        }

        // Normal navigation for other types (res_sub_type === 0)
      ...
      renderActions: (
        ...
        // @ts-ignore - source_type field might not exist yet
        if (item.source_type == 'corekg') {
          ...
          const deleteProps = {
            ...
            handler: async () => {
              ...
              await KnowledgeApi.DeleteDataset({ dataset_id: item.res_id });
              setTimeout(() => {
                reloadList();
              }, 500);
              Toast.success(I18n.t('Delete_success'));
          ...
          return <TableAction deleteProps={deleteProps} actionList={[]} />;
        }
