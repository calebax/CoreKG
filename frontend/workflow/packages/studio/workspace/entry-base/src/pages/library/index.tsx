...
import { useLibraryResourcePermissions } from './hooks/use-library-resource-permissions';
...
>(
  ...
  ({ spaceId, isPersonalSpace = true, entityConfigs, onResourceListChange }, ref) => {
    ...
    const filteredDataSource = useMemo(() => {
      return (
        listResp.data?.list?.filter(
          item => !optimisticallyDeletedIds.has(item.res_id || ''),
        ) || []
      );
    }, [listResp.data?.list, optimisticallyDeletedIds]);

    // 获取权限信息
    const { canManageResource, permissionsLoading } =
      useLibraryResourcePermissions(listResp.data?.list);

    // 只有在权限加载完成后才显示数据，避免操作列"一闪而过"
    const finalDataSource = useMemo(() => {
      // 如果权限正在加载中，返回空数组，不渲染任何数据
      if (permissionsLoading) {
        return [];
      }
      return filteredDataSource;
    }, [permissionsLoading, filteredDataSource]);
    ...
    }, [listResp.data?.list, onResourceListChange]);
    ...
    const columns = useGetColumns({
      ...
      isPersonalSpace,
      onOptimisticDelete: (resourceId: string) => {
        setOptimisticallyDeletedIds(prev => new Set([...prev, resourceId]));
      },
      canManageResource,
    });

    const typeFilterData = useMemo(() => {
      // 显示的资源库类型: All Types, Plugin, Workflow, Prompt
      const allowedTypes = [ResType.Plugin, ResType.Workflow, ResType.Prompt];

      return [
        { label: I18n.t('library_filter_tags_all_types'), value: -1 },
        ...entityConfigs
          .filter(item => item.target.some(t => allowedTypes.includes(t)))
      ...
      >
        ...
        <Layout.Content>
          <Table
            ...
            offsetY={178}
            tableProps={{
              loading: listResp.loading || permissionsLoading,
