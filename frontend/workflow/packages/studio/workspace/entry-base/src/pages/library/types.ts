...
export interface LibraryEntityConfig {
   ...
   * @param onOptimisticDelete 乐观UI更新回调
   * @Returns render result
   */
  renderActions: (item: ResourceInfo, reloadList: () => void, onOptimisticDelete?: (resourceId: string) => void) => ReactNode;
...
export interface BaseLibraryPageProps {
  ...
  onResourceListChange?: (names: string[]) => void;
}
