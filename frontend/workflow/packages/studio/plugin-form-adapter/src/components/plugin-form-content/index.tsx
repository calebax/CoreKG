...
export const PluginForm: FC<{
  ...
  onNameValidationChange?: (values?: Record<string, unknown>) => void;
}> = ({
  pluginState,
  disabled,
  editInfo,
  ...
  const changeVal = (values?: Record<string, unknown>) => {
  ...
  const reset = () => {
    //Reset plugin
    getIcon();
    formApi.current?.setValue('creation_method', CLOUD_PLUGIN_COZE);
    formApi.current?.setValue('auth_type', [0]);
    setAuthType(0);
    setMainAuthType(0);
    setExtItems([]);
    setHeaderList([{ name: 'User-Agent', value: 'Coze/1.0' }]);
    setIsValidCheckResult(true);
    setPluginTypeCreationMethod(CLOUD_PLUGIN_COZE);
    onNameValidationChange?.({
      ...(formApi.current?.getValues?.() ?? {}),
      creation_method: CLOUD_PLUGIN_COZE,
      auth_type: [0],
    });
  };
  ...
  return (
    ...
    >
      ...
      <PictureUpload
        ...
        onChange={() => {
          changeVal(formApi.current?.getValues?.() ?? {});
        }}
      />
      ...
      <>
        <FormTextArea
          ...
          rows={1}
          onBlur={async () => {
            const trimmedName = formApi.current?.getValue('name')?.trim();
            formApi.current?.setValue(
              'name',
              trimmedName,
            );
        ...
        <FormTextArea
          ...
          maxLength={600}
          onBlur={() => {
            const trimmedDesc = formApi.current?.getValue('desc')?.trim();
            formApi.current?.setValue(
              'desc',
              trimmedDesc,
            );
        ...
        >
          {isCreate ? (
            <Form.RadioGroup
              ...
              initValue={
                editInfo ? doFormatTypeAndCreation(editInfo) : CLOUD_PLUGIN_COZE
              }
              onChange={v => {
                setPluginTypeCreationMethod(v.target.value);
                onNameValidationChange?.({
                  ...(formApi.current?.getValues?.() ?? {}),
                  creation_method: v.target.value,
                });
              }}
            />
          ) : (
