...
const isPromptFormSubmittable = ({
  ...
  if (isDuplicateName(name, existingNames, excludeName)) {
    return false;
  }
...
export const PromptConfiguratorModal = (
  ...
  const [isSubmitDisabled, setIsSubmitDisabled] = useState(mode !== 'info');
  ...
  const updateSubmitDisabled = (partialValues?: Partial<PromptValues>) => {
    const nextValues = {
      ...formValues,
      ...(formApiRef.current?.getValues?.() as Partial<PromptValues> | undefined),
      ...partialValues,
    } as PromptValues;

    setIsSubmitDisabled(
      !isPromptFormSubmittable({
        mode: modalMode,
        values: nextValues,
        existingNames,
        excludeName: modalMode === 'edit' ? originalNameRef.current : '',
      }),
    );
  };
  ...
  const handleUpdateModeAction = async (
    ...
    try {
      ...
      if (mode === 'create' && id) {
        const retryRefresh = async (retryCount = 0) => {
          const maxRetries = 3;
          const delay = 500 + retryCount * 200;
          
          setTimeout(async () => {
            try {
              await PlaygroundApi.GetPromptResourceInfo({
    ...
    PlaygroundApi.GetPromptResourceInfo({
      ...
      ({ data: { name = '', description = '', prompt_text = '' } = {} }) => {
        ...
        setIsSubmitDisabled(
          !isPromptFormSubmittable({
            mode: modalMode,
            values: { name, description, prompt_text },
            existingNames,
            excludeName: name,
          }),
        );
  ...
  }, [editId, modalMode, editor, existingNames]);

  useEffect(() => {
    updateSubmitDisabled();
  }, [modalMode, existingNames]);
  ...
  return (
    ...
    >
      <Modal
        ...
        width="640px"
        footer={
          <div className="flex items-center justify-end prompt-modal-footer-buttons">
            {enableDiff ? (
              <PromptDiff
                spaceId={spaceId}
                botId={botId}
          ...
          <div>
            ...
            >
              <PromptInfoInput
                ...
                onBlur={async () => {
                  updateSubmitDisabled({
                    name: formApiRef.current?.getValue('name'),
                  });
                }}
                onChange={value => {
                  setFormValues(prev => ({ ...prev, name: value }));
                  updateSubmitDisabled({ name: value });
                }}
                rules={[
                  {
                    required: !isReadOnly,
                  ...
                  ...(existingNames.length && modalMode === 'create'
                    ? [
                        {
              ...
              <PromptInfoInput
                ...
                onChange={value => {
                  ...
                  updateSubmitDisabled({ description: value });
                }}
              />
                ...
                >
                  <PromptEditorRender
                    ...
                    onChange={value => {
                      ...
                      setFormValues(prev => ({ ...prev, prompt_text: value }));
                      updateSubmitDisabled({ prompt_text: value });
                    }}
                  />
                  <InputSlotWidget
                    mode="configurable"
