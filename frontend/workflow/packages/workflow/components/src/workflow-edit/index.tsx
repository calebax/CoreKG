...
const hasWorkflowNameValidationError = ({
  name,
  nameValidators,
}: {
  name: string;
  ...
  return nameValidators.some(validator => {
    const result = validator.validator([], name);
...
export function CreateWorkflowModal({
  ...
  const lastNameRef = useRef(workFlow?.name?.trim() || '');
  ...
  const updateConfirmDisabled = async () => {
    const formApi = formRef.current?.formApi;
    const values = formApi?.getValues() as Partial<FormValue> | undefined;
    ...
    if (
      hasWorkflowNameValidationError({
        name,
        nameValidators,
      })
    ) {
  ...
  return (
    <UIModal
      ...
      title={title}
      footer={
        <Space className="workflow-modal-footer-buttons">
          <Button
      ...
      <Form<Partial<FormValue>>
        ...
        onValueChange={({ name, target }) => {
          ...
          if (
            ...
            normalizedName !== lastNameRef.current
          ) {
            lastNameRef.current = normalizedName;
            setConfirmDisabled(true);
            return;
          }
