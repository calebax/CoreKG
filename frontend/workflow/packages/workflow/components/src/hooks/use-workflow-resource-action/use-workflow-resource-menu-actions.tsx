  ...
  const renderWorkflowResourceActions = (record: ResourceInfo): ReactNode => {
    ...
    return (
      <div className="flex items-center gap-3">
        {/* 编辑按钮 */}
        {extraActions
          .filter(action => !action.hide && action.actionKey === 'edit')
          .map(action => (
            <span
        ...
        {/* 复制按钮 */}
        {!copyActionConfig?.hide && (
          <span
            onClick={e => {
              e.stopPropagation();
              if (copyActionConfig?.enable !== false) {
                actionMap[ActionKey.Copy]?.(record);
              }
            }}
            className={`text-sm ${
              copyActionConfig?.enable === false
                ? 'text-[var(--coz-fg-dim)] cursor-not-allowed'
                : 'text-[var(--coz-fg-primary)] cursor-pointer hover:text-[var(--coz-fg-hglt)]'
            }`}
          >
        ...
        {/* 删除按钮 */}
        {!deleteActionConfig?.hide && (
          <span
            onClick={e => {
              e.stopPropagation();
              if (deleteActionConfig?.enable !== false) {
                actionMap[ActionKey.Delete]?.(record);
              }
            }}
            className={`text-sm ${
              deleteActionConfig?.enable === false
                ? 'text-[var(--coz-fg-dim)] cursor-not-allowed'
                : 'text-[var(--coz-fg-primary)] cursor-pointer hover:text-[var(--coz-fg-hglt)]'
            }`}
          >
