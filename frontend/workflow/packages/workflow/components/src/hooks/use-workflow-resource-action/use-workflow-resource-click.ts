...
export const useWorkflowResourceClick = (spaceId?: string) => {
  ...
  const handleWorkflowResourceClick = (record: ResourceInfo) => {
    ...
    let effectiveSubType = record?.res_sub_type;
    if (record?.res_sub_type === 11) {
      effectiveSubType = 0;
    } else if (record?.res_sub_type === 12) {
      effectiveSubType = 1;
    }

    const modifiedRecord = {
      ...record,
      res_sub_type: effectiveSubType,
    };

    onEditWorkFlow(modifiedRecord?.res_id);
  };
