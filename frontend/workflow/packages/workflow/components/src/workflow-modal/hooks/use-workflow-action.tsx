...
export function useWorkflowAction({
  ...
  function openWorkflowDetailPage(workflow: WorkflowInfo | ProductInfo) {
    ...
    // 直接使用完整URL，避免路径重复
    window.open(
      `${window.location.origin}/coze/work_flow?${query.toString()}`,
      '_blank',
    );
  }
          ...
          content: (
            <Space spacing={6}>
              ...
              <Button
                ...
                onClick={() => {
                  window.open(
                    `/coze/work_flow?space_id=${context.spaceId}&workflow_id=${newWorkflow.workflow_id}&from=dupSuccess`,
                  );
                }}
              >
          ...
          content: (
            <Space spacing={6}>
              ...
              <Button
                ...
                onClick={() => {
                  window.open(
                    `/coze/work_flow?space_id=${context.spaceId}&workflow_id=${newWorkflow.workflow_id}`,
                  );
                }}
              >
                {I18n.t('workflowstore_continue_editing')}
