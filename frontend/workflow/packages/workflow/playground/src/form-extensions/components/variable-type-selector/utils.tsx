...
const getArrayChildren = (
  ...
  // 根据环境配置判断是否隐藏File类型入口（custom环境下hideFileType为true时隐藏）
  const isFileTypeHidden =
    window.__CONFIG?.version === 'custom' &&
    window.__CONFIG?.hideFileType === true;
  const baseTypes = [
...
export const allVariableTypeList: Array<VariableTypeOption> = [
  ...
  // 根据环境配置判断是否隐藏顶层File类型入口（custom环境下hideFileType为true时隐藏）
  ...(window.__CONFIG?.version === 'custom' &&
  window.__CONFIG?.hideFileType === true
    ? []
    : [
