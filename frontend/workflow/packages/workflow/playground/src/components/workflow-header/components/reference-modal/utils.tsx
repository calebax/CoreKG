...
export const navigateResource = ({
  ...
  if (info.from === DependencyOrigin.LIBRARY) {
    switch (info.type) {
      case NodeType.PLUGIN:
        window.open(`/coze/space/${spaceId}/plugin/${info.id}`);
        break;
      case NodeType.WORKFLOW:
      case NodeType.CHAT_FLOW:
        window.open(`/coze/work_flow?space_id=${spaceId}&workflow_id=${info.id}`);
        break;
      case NodeType.KNOWLEDGE:
        window.open(`/coze/space/${spaceId}/knowledge/${info.id}`);
        break;
      case NodeType.DATABASE:
        window.open(`/coze/space/${spaceId}/database/${info.id}`);
        break;
      default:
        return;
    }
  ...
  if (info.type === NodeType.PLUGIN && info.from === DependencyOrigin.SHOP) {
    window.open(`/coze/store/plugin/${info.id}`);
  }
};
