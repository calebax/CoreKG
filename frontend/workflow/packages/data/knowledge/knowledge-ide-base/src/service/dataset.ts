...
/** Resolve dataset detail when API keys may be string/number variants of dataset_id */
export const getDatasetDetailFromResponse = (
  datasetDetails: Record<string | number, Dataset> | undefined,
  datasetID: string,
): Dataset | undefined => {
  if (!datasetDetails || !datasetID) {
    return undefined;
  }
  const direct = datasetDetails[datasetID];
  if (direct) {
    return direct;
  }
  const numericKey = Number(datasetID);
  if (!Number.isNaN(numericKey)) {
    const byNumericKey = datasetDetails[numericKey];
    if (byNumericKey) {
      return byNumericKey;
    }
  }
  return Object.values(datasetDetails).find(
    item => String(item.dataset_id) === String(datasetID),
  );
};
  ...
  const { data, run, loading, mutate } = useRequest(
    async ({ datasetID }: { datasetID: string }) => {
      ...
      try {
        ...
        const curDatasetDetail = getDatasetDetailFromResponse(
          res?.dataset_details,
          datasetID,
        );
        if (curDatasetDetail) {
          ...
          // 当接口返回成功但找不到对应的数据集详情时，抛出错误
          const errorMsg = `Dataset not found: ${datasetID}`;
          const error = new CustomError('useDataSetDetailReq_dataset_not_found', errorMsg);
