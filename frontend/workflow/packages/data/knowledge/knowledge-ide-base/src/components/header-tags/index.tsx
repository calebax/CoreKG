  ...
  return (
    <div className="flex pb-[4px]" data-testid={KnowledgeE2e.UnitDetailTags}>
      <Space spacing={4} className="[&_.semi-tag-content]:font-medium">
        ...
        {/* doc count */}
        {(formatType === FormatType.Text || docInfo?.format_type === 11 || docInfo?.format_type === 12) && !!dataSetDetail?.doc_count && (
          <Tag size="mini" color="primary">
            {I18n.t('kl2_009', {
        ...
        {/* 多模态 */}
        {/* corekg同步表格 */}
        {formatType !== FormatType.Image && docInfo?.format_type !== 11 && docInfo?.format_type !== 12 && !!dataSetDetail?.doc_count  && (
          <>
            {/* slice count  */}
            <Tag size="mini" color="primary">
