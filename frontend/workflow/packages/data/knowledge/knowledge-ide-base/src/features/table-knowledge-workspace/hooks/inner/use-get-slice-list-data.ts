...
import { useKnowledgeStore, useKnowledgeParams } from '@coze-data/knowledge-stores';
...
export const useGetSliceListData = () => {
  ...
  // 确保document_id有效，documentList已加载且不为空数组，如果没有文档则不发起请求
  ...
  const {
    ...
    reloadDeps: [curDocId, datasetID, documentList, shouldFetchData],
    target: null,
    dataReady: shouldFetchData,
    onError: error => {
