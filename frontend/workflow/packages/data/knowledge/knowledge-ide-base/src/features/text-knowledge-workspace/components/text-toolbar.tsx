...
export const TextToolbar: React.FC<TextToolbarProps> = ({
  ...
  const shouldRenameDoc =
    canRenameDoc ?? (canEdit && curFormatType !== FormatType.CoreKG_File);
  ...
  return (
    ...
    >
      ...
      <Space spacing={8} className={classnames({ hidden: forbidSwitch })}>
        {fileUrl || dataSetDetail?.format_type === 11 ? (
          <div className="flex items-center gap-2">
            ...
            <Switch
              ...
              onChange={(...args) => {
                onToggleOriginalFile(!showOriginalFile);
              }}
            ></Switch>
          </div>
        ) : null}
        {/* 多模态 */}
        {showResegmentButton && canEdit && dataSetDetail?.format_type !== 11 ? (
        ...
        {linkOriginUrlButton}
        {/* 多模态 */}
        {showDeleteDocBtn && dataSetDetail?.format_type !== 11 ? (
