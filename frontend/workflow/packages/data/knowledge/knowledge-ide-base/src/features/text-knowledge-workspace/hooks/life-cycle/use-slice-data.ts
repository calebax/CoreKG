...
export const useSliceData = ({
  ...
  // 确保document_id有效，如果没有文档则不发起请求
  const shouldFetchData = Boolean(
    datasetId && 
    (curChunkType !== ChunkType.LevelChunk ? 
      curDocId && curDocId.trim() !== '' && curDocId !== 'undefined' : 
      true)
  );
  ...
  const {
    ...
    params: {
      ...
      document_id:
        ...
        curChunkType !== ChunkType.LevelChunk && curDocId && curDocId.trim() !== '' && curDocId !== 'undefined' 
          ? curDocId 
          : undefined,
