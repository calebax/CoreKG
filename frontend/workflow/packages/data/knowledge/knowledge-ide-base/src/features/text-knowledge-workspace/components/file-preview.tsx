...
export const FilePreview: React.FC<FilePreviewProps> = ({
  ...
  const shouldUsePdfPreview = isOfficePdfPreviewType(normalizedFileType);
  const {
    pdfNode,
    numPages,
    currentPage,
    ...
    fileUrl: shouldUsePdfPreview ? fileUrl : '',
  });
    ...
    >
      {(() => {
        switch (normalizedFileType) {
          ...
          case 'xlsx': {
            return <XLSXPreviwer file={fileUrl} />;
          }
          case 'docx':
          case 'pdf':
          case 'doc':
          ...
          default: {
            if (isImagePreviewType(normalizedFileType)) {
              return <PreviewImage fileUrl={fileUrl} />;
            }
            if (isAudioPreviewType(normalizedFileType)) {
              return <PreviewAudio fileUrl={fileUrl} />;
            }
            if (isVideoPreviewType(normalizedFileType)) {
              return <PreviewVideo fileUrl={fileUrl} />;
            }
            return <PreviewUrl fileUrl={fileUrl} />;
          }
        }
      })()}
    </div>
...
const XLSXPreviwer = lazy(() => import('./XLSXViewer'));
