...
const useKnowledgeFilter = ({
  ...
  knowledgeTypeConfigList = [
    ...
    // 增加多模态类型到默认配置列表
    FilterKnowledgeType.MULTIMODAL,
    FilterKnowledgeType.QAPAIR,
  ],
  projectID,
  beforeCreate,
              ...
              currentKnowledgeType === FilterKnowledgeType.ALL
                ? undefined
                  : {
                    ...
                    // 多模态类型映射
                    [FilterKnowledgeType.MULTIMODAL]: FormatType.CoreKG_File || 11,
                    [FilterKnowledgeType.QAPAIR]: FormatType.Qapair || 13,
  ...
  const renderKnowledgeTypeConfigNode = (type: FilterKnowledgeType) => {
    ...
    // 增加多模态 Tab 渲染逻辑
    if (type === FilterKnowledgeType.MULTIMODAL) {
      return (
        <div
          data-testid={BotE2e.BotKnowledgeSelectListModalMultimodalTab} // 假设的 testid
          key={FilterKnowledgeType.MULTIMODAL}
          onClick={() => setCurrentKnowledgeType(FilterKnowledgeType.MULTIMODAL)}
          className={
            currentKnowledgeType === FilterKnowledgeType.MULTIMODAL
              ? styles['file-type-tab-item-active']
              : styles['file-type-tab-item']
          }
        >
    ...
    if (type === FilterKnowledgeType.QAPAIR) {
      return (
        <div
          data-testid={BotE2e.BotKnowledgeSelectListModalMultimodalTab} // 假设的 testid
          key={FilterKnowledgeType.QAPAIR}
          onClick={() => setCurrentKnowledgeType(FilterKnowledgeType.QAPAIR)}
          className={
            currentKnowledgeType === FilterKnowledgeType.QAPAIR
              ? styles['file-type-tab-item-active']
              : styles['file-type-tab-item']
          }
        >
            ...
            <div className="flex gap-[8px]">
              {uniq(showFilters).map((filterType: DatasetFilterType) => {
                if (filterType === 'scope-type') {
                  ...
                  // Convert FilterKnowledgeType to res_sub_type for UISearch
                  const getResSubType = (
                    type: FilterKnowledgeType,
                  ): number | undefined => {
                    switch (type) {
                      case FilterKnowledgeType.TEXT:
                        return FormatType.Text; // 0
                      case FilterKnowledgeType.TABLE:
                        return FormatType.Table; // 1
                      case FilterKnowledgeType.IMAGE:
                        return FormatType.Image; // 2
                       // 搜索框增加多模态类型支持
                      case FilterKnowledgeType.MULTIMODAL:
                        return FormatType.CoreKG_File; // 11
                      case FilterKnowledgeType.QAPAIR:
                        return FormatType.Qapair; // 13
                      default:
                        return undefined; // For ALL type, don't hide search
                    }
                  };
                  ...
                  return (
                    <UISearch
                      ...
                      res_sub_type={getResSubType(currentKnowledgeType)}
                    />
                  );
                }
              })}
