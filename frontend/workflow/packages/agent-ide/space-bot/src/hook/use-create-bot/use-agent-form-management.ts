...
export interface UseAgentFormManagementProps {
  ...
  requireNameBlurValidation?: boolean;
}

export const useAgentFormManagement = ({
  initialBotInfo,
  requireNameBlurValidation = false,
}: UseAgentFormManagementProps) => {
  const formRef = useRef<Form<AgentInfoFormValue>>(null);
  const lastNameRef = useRef(initialBotInfo?.name?.trim() || '');
  ...
  const resetFormState = () => {
    lastNameRef.current = initialBotInfo?.name?.trim() || '';
  ...
  const handleFormValuesChange = (values: AgentInfoFormValue) => {
    ...
    setCheckErr(false);
    setErrMsg('');
    if (currentName !== lastNameRef.current) {
      lastNameRef.current = currentName;
      if (requireNameBlurValidation) {
        setHasNameValidationError(false);
      }
    }
    setOkButtonDisable(!currentName);
  };
