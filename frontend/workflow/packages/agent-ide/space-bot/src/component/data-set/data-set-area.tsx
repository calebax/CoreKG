...
import { Tooltip, Popover, Spin } from '@coze-arch/coze-design';
...
const E2E_NAME_MAP = {
  ...
  [FormatType.CoreKG_File]: 'multimodal',
  [FormatType.Qapair]: 'qapair',
};
...
export const DataSetAreaItem: FC<IDataSetAreaProps> = ({
  ...
  const defaultKnowledgeType = useMemo(() => {
    switch (formatType) {
      ...
      case FormatType.CoreKG_File:
        return FilterKnowledgeType.MULTIMODAL;
      case FormatType.Qapair:
        return FilterKnowledgeType.QAPAIR;
      default:
        return undefined;
    }
  }, [formatType]);
  ...
  const jumpToDetail = (datasetID: string) => {
    const dataset = dataSetList.find(item => item.dataset_id === datasetID);
    const actionType = dataset ? ActionType.REMOVE : ActionType.ADD;
    ...
    if (dataset?.format_type === FormatType.CoreKG_File) {
      const forestId =
        (dataset as { corekg_detail_id?: number }).corekg_detail_id ??
        datasetID;
      queryParams.forestId = String(forestId);
      if (dataset.name) {
    ...
    () =>
      dataSetList.filter(item => {
        ...
        if (formatType === FormatType.Text) {
          return item.format_type === FormatType.Text;
        }

        if (formatType === FormatType.Table) {
          return item.format_type === FormatType.Table || item.format_type === FormatType.CoreKG_Excel;
        }

        if (formatType === FormatType.CoreKG_File) {
          return item.format_type === FormatType.CoreKG_File;
        }

        if (formatType === FormatType.Qapair) {
          return item.format_type === FormatType.Qapair;
        }

        return item.format_type === formatType;
      }),
    [dataSetList, formatType],
  );
  ...
  const showKnowledgeLoading =
    isDetailLoading &&
    knowledge.dataSetList.length > 0 &&
...
export const useDataSetArea = () => {
  ...
  const getDataSetList = async () => {
    const boundList = useBotSkillStore.getState().knowledge.dataSetList;
    if (!boundList.length) {
