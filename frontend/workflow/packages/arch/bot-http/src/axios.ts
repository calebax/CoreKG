    ...
    if (code !== 0) {
      ...
      switch (code) {
        case ErrorCodes.NOT_LOGIN: {
          ...
          if (isInIframe()) {
            notifyAuthError({ code, message: message ?? msg });
            requestRelogin();
          } else {
  ...
  error => {
    if (isAxiosError(error)) {
      ...
      if (error.response?.status === HTTP_STATUS_COE_UNAUTHORIZED) {
        if (isInIframe()) {
          notifyAuthError({ code: 401, message: 'Unauthorized in iframe' });
          requestRelogin();
...
axiosInstance.interceptors.request.use(config => {
  ...
  try {
    ...
    if (syncedToken) {
      ...
      // 2. 降级策略：如果在非嵌入环境（独立开发），尝试读取本地开发环境的 Store
      const localStoreItem = localStorage.getItem('ai-yygu-local-storage');
      if (localStoreItem) {
        try {
          // 增加 try-catch 防止 JSON 解析失败导致请求中断
          const token = JSON.parse(localStoreItem)?.state?.token;
          if (token) {
