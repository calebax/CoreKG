...
import { appendUrlParam, getParamsFromQuery, openUrl, getAppBasePath, withAppBasePath } from '../src/url';
...
describe('URL', () => {
  ...
  it('#getAppBasePath', () => {
    window.location.pathname = '/coze/work_flow';
    expect(getAppBasePath()).toEqual('/coze');

    window.location.pathname = '/coze/space/2/library';
    expect(getAppBasePath()).toEqual('/coze');

    window.location.pathname = '/work_flow';
    expect(getAppBasePath()).toEqual('/coze');
  });

  it('#withAppBasePath', () => {
    window.location.pathname = '/coze/work_flow';
    expect(withAppBasePath('/space/2/library?type=1')).toEqual(
      '/coze/space/2/library?type=1',
    );
  });
});
