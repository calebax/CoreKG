...
export const LineChartForm: React.FC<LineChartFormProps> = ({
  config,
  onChange,
  chartType,
  onChartTypeChange,
  ...
  // 更新系列数据
  const updateSeriesData = (seriesIndex: number, field: string, value: any) => {
    const newConfig = { ...config }
    if (!newConfig.series) newConfig.series = []
    if (!newConfig.series[seriesIndex]) {
      newConfig.series[seriesIndex] = { type: chartType, data: [] }
    }
    newConfig.series[seriesIndex][field] = value
    onChange(newConfig)
  }
    ...
    <Space direction='vertical' className='w-full' size='large'>
      ...
      <div>
        ...
        <Form.Item label='平滑曲线'>
          <Switch
            size='small'
            checked={Boolean(getConfigValue(['series', '0', 'smooth'], false))}
            onChange={(checked) => updateSeriesData(0, 'smooth', checked)}
          />
        </Form.Item>
      </div>
    </Space>
