...
import { BaseType, Selection, pointer, select as d3Select } from 'd3-selection'
import {
  D3ZoomEvent,
  ZoomBehavior,
  zoom as d3Zoom,
...
export class Visualization {
  ...
  /** 创建关系的起点节点 null表示未处于创建状态 */
  public edgeStartNode: NodeModel | null = null

  // 鼠标位置（container坐标系）
  private mousePosition: { x: number; y: number } | null = null

  // 临时箭头对象
  private tempArrow: StraightArrow | null = null
  ...
  constructor(
    ...
    public showNodeOperators?: boolean,
    ...
    this.root
      ...
      // 添加鼠标移动事件监听，用于追踪鼠标位置（创建关系时显示预览箭头）
      .on('mousemove', (event: MouseEvent) => {
        // 使用 pointer 函数将鼠标坐标转换为 container 坐标系
  ...
  renderTempArrow(): void {
    // 如果没有起点节点或鼠标位置，清理临时箭头
    if (!this.edgeStartNode || !this.mousePosition) {
      this.container.select('g.layer.temp-arrow').remove()
      this.tempArrow = null
      return
    }

    const startNode = this.edgeStartNode
    const mousePos = this.mousePosition

    // 计算节点中心到鼠标位置的距离和角度
    const dx = mousePos.x - startNode.x
    const dy = mousePos.y - startNode.y
    const distance = Math.sqrt(dx * dx + dy * dy)
    const angle = (Math.atan2(dy, dx) * 180) / Math.PI

    // 创建临时箭头对象 - 从节点中心开始
    this.tempArrow = new StraightArrow(
      0, // 起点半径
      0, // 终点半径
      distance, // 中心距离
      5, // 箭头宽
      13, // 箭头头部宽度
      13, // 箭头头部高度
      'internal', // captionLayout
    )

    // 确保临时箭头层存在
    let tempArrowLayer =
      this.container.select<SVGGElement>('g.layer.temp-arrow')
    if (tempArrowLayer.empty()) {
      tempArrowLayer = this.container
        .append('g')
        .attr('class', 'layer temp-arrow')
    }

    // 渲染箭头路径 - 使用亮蓝色
    const arrowColor = '#00BFFF' // 亮蓝色 (DeepSkyBlue)
    tempArrowLayer
      .selectAll<SVGPathElement, unknown>('path.temp-arrow')
      .data([this.tempArrow])
      .join('path')
      .attr('class', 'temp-arrow')
      .attr(
        'transform',
        `translate(${startNode.x} ${startNode.y}) rotate(${angle})`,
      )
      .attr('d', (arrow) => arrow.outline(0))
      .attr('fill', arrowColor)
      .attr('stroke', 'none')
      .style('opacity', 0.8) // 稍微提高不透明度以显示亮蓝色
  }
  ...
  init(): void {
    this.container
      .selectAll('g.layer')
      .data(['relationships', 'nodes', 'temp-arrow'])
      .join('g')
  ...
  precomputeAndStart(): void {
    this.forceSimulation.precomputeAndStart(() => this.zoomByType(ZoomType.FIT))
  }
