...
export const nodeEventHandlers = (
  ...
  visualization?: { edgeStartNode: NodeModel | null },
) => {
  ...
  const onNodeClick = (_event: Event, node: NodeModel) => {
    // 如果正在创建关系，结束创建并传输终点节点
    if (visualization?.edgeStartNode !== null) {
      trigger('clickOperator:stopCreateEdge', node)
      return
    }
  ...
  const onNodeDblClick = (_event: Event, node: NodeModel) => {
    if (visualization?.edgeStartNode) {
      return
    }
