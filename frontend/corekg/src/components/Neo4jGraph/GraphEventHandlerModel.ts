 ...
 */
import { OperatorClickType } from './index'
...
export class GraphEventHandlerModel {
  ...
  constructor(
    ...
    this.onOperatorsClick = onOperatorsClick
  ...
  onClickOperators(
    ...
    this.onOperatorsClick?.(type, node, endNode)
  }
  ...
  bindEventHandlers(): void {
    this.visualization
      .on('click', () => {
        // 点击时 结束关系链接
        if (this.visualization.edgeStartNode) {
          this.visualization.edgeStartNode = null
          this.visualization.update({
            updateNodes: true,
      ...
      .on('clickOperator:editNode', (node: NodeModel) =>
        this.onClickOperators('editNode', node),
      )
      .on('clickOperator:deleteNode', (node: NodeModel) =>
        this.onClickOperators('deleteNode', node),
      )
      .on('clickOperator:startCreateEdge', (node: NodeModel) => {
        setTimeout(() => {
          this.visualization.edgeStartNode = node
          this.visualization.update({
            updateNodes: true,
            updateRelationships: false,
            restartSimulation: false,
          })
          this.visualization.renderTempArrow()
        }, 0)
        // 取消由click事件处理
      })
      .on('clickOperator:stopCreateEdge', (node: NodeModel) => {
        // 如果用户点击了node 触发边的编辑
        if (this.visualization.edgeStartNode) {
          this.onClickOperators(
            'createEdge',
            this.visualization.edgeStartNode!,
            node,
          )
        }
      })
