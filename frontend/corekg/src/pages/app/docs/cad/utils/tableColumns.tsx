      ...
      if (canParse) {
        return (
          <div className='flex items-center'>
            ...
            <Tooltip title={title}>
              <span
                className={`${record.isFolder && handleFolderClick && currentLevel < 4 ? 'cursor-pointer hover:text-[#4080FF]' : !record.isFolder && handleFileClick ? 'cursor-pointer hover:text-[#4080FF]' : ''}`}
                onClick={() => {
