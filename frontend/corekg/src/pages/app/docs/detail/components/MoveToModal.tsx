...
import { Modal, Button, message, Input, Tooltip } from 'antd'
...
import DropIcon from '../images/drop-icon.svg?react'
import { FileItem } from '../types'
import DbIcon from './images/db.svg?react'
import ExcelIcon from './images/excel.svg?react'
import OdsIcon from './images/ods.svg?react'
import QaIcon from './images/qa.svg?react'
...
const MoveToModal: React.FC<MoveToModalProps> = ({
        ...
        const folders = [
          {
            ...
            size: '-',
            updatedAt: knowledgeBaseCreatedAt || new Date().toISOString(),
            isFolder: true,
            level: 0,
            expanded: true,
            children: folderList,
  ...
  const canMoveToFolder = (folder: FolderTreeItem): boolean => {
    ...
    return targetLevel <= 4 && folder.id !== selectedItem.id // 最大层级是4（第5层）
  }
  ...
  const handleFolderClick = (
    ...
    if (!canMoveToFolder(folder)) {
      if (folder.id === selectedItem?.id)
        return message.warning('无法移动至自己的子级目录')
      message.warning(
  ...
  const iconMap: Record<
    MoveToModalProps['knowledgeBaseType'],
  ...
  const renderIcon = () => {
    const Component = iconMap[knowledgeBaseType]
    return <Component className={`w-5 h-5 mr-2`} />
  }
  ...
  const renderFolderTree = (folderArray: FolderTreeItem[]) => {
    ...
    return flatFolders.map((folder, index) => {
      ...
      const isCurrent = folder.id === selectedItem?.id

      if (isCurrent) {
        return (
          <Tooltip title='无法移动至自己的子级目录'>
            <div key={`folder-item-${folder.id}`}>
              {/* 文件夹行 */}
              <div
                className={`flex items-center h-[30px] px-3 rounded-md ${canMove ? `cursor-pointer hover:bg-[#E7F5FF] hover:text-[#0c99ff] ${isSelected ? 'bg-[#E8F3FF] text-[#0c99ff]' : ''}` : 'cursor-not-allowed opacity-50 bg-gray-50'}`}
              ...
              >
                ...
                >
                  <DropIcon
                    className={`w-[14px] h-[14px] ${folder.expanded ? 'transform-[rotate(0deg)]' : 'transform-[rotate(-90deg)]'}`}
                  />
                </div>
                {folder.level === 0 ? (
                  renderIcon()
                ...
                {editingId === folder.id ? (
                  ...
                  <span
                    className={`flex-1 truncate font-normal text-base ${canMove ? '' : 'text-gray-400'} ${isSelected ? 'bg-[#E8F3FF] text-[#0c99ff]' : ''}`}
                  >
                    {folder.name}
  ...
  return (
    ...
    >
      <div>
        ...
        {/* 分割线 */}
        <div className='h-[1px] w-full bg-[#E5E6EB] mb-2 mt-4'></div>
        ...
        {/* 底部按钮 */}
        <div className='flex justify-end border-t border-[#E5E6EB] pt-4'>
          {/* <Button
          ...
          <div className='flex gap-3 '>
            {/* 取消 */}
            <Button
              className='!h-[32px] !px-[15px]  !bg-[#F5F5F5] hover:!bg-[#F5F5F5] !border-none !rounded-[6px] !text-[#0C1F17] !font-[500] !text-[14px]'
              onClick={onCancel}
            >
              取消
            </Button>
            ...
            {/* 移动至此 */}
            <Button
              className='!h-[32px] !px-[15px] !bg-[#0C99FF] hover:!bg-[#0C99FF] !text-[#fff] !rounded-[6px]  !text-[14px] !font-[500] '
              onClick={handleMove}
              loading={loading}
            >
              移动
