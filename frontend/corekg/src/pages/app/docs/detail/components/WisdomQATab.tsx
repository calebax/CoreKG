...
import { getFileInfo, deleteFileQA } from '@/api/knowledge'
...
export default function WisdomQATab() {
  ...
  const { data: fileInfo, loading: fileInfoLoading } = useRequest(
    async () => {
  ...
  const knowledgeStatus = fileInfo?.knowledge_status
  ...
  const loading = historyLoading || fileInfoLoading || isAnswering

  const isBtnActive = useMemo(() => {
    return Boolean(text?.trim() && !loading && knowledgeStatus === 'success')
  }, [text, loading, knowledgeStatus])
  ...
  return (
    <div className='h-full flex flex-col gap-3 overflow-hidden relative'>
      ...
      >
        {dialog.length === 0 && knowledgeStatus === 'success' ? (
          ...
          <div className='relative flex items-center gap-3'>
            ...
            <div className='flex-1 relative'>
              <TextArea
                ...
                disabled={loading || knowledgeStatus !== 'success'}
                style={{
                  borderRadius: '999px',
                  paddingLeft: '20px',
                  paddingRight: '56px',
              ...
              <button
                onClick={onQA}
                disabled={
                  !text.trim() || loading || knowledgeStatus !== 'success'
                }
