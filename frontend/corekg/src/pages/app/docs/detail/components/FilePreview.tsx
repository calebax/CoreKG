...
import { useFileLocation } from '@/pages/app/docs/utils'
...
const FilePreview: React.FC<FilePreviewProps> = ({
  ...
  const renderFilePreview = () => {
    ...
    switch (detectedFileType) {
      case 'pdf':
        // location[0] 规则：
        // - 为 0 时表示第一页，需要转换为 PDF 页码 1
        // - 大于 0 时直接作为 PDF 页码使用
        let pageNumber: number | undefined
        if (fileLocation && fileLocation.length > 0) {
          const firstLocation = fileLocation[0]
          if (firstLocation === 0) {
            pageNumber = 1
          } else if (typeof firstLocation === 'number' && firstLocation > 0) {
            pageNumber = firstLocation
          }
        }
        ...
        return (
          <PDFViewer
            ...
            id={fileId!}
            name={fileName}
            locationKey={location.search} // 传递 location.search 用于跟踪URL变化
