...
export default function AddMembersModal({
  ...
  const getSelectedMembers = () => {
    ...
    return selectedIds.map((employeeId) => {
      ...
      const user = userList.find(
        (userItem: UserListItem) => userItem.value === employeeId,
      )
      return {
        id: employeeId,
  ...
  // 递归获取部门下的所有员工ID
  const getAllEmployeesInDepartment = (
    departmentId: number,
  ...
  const isDepartmentFullySelected = (
    departmentId: number,
  ...
  const getSelectedDepartmentIds = (
    ...
    return department
      .filter((dept) =>
        isDepartmentFullySelected(Number(dept.id), selectedEmployeeIds),
      )
    ...
    if (type === 'check') {
      ...
      if (node.type === 'employee') {
        ...
        if (searchValue) {
          ...
          const { data } = usePersonnelData.getState()
          const employeesInSearchResult =
            data.employee?.filter((emp) => {
              // 检查员工是否属于该部门且姓名匹配搜索
              return (
                emp.departmentIds.includes(Number(node.id)) &&
      ...
      // 只添加尚未选中的员工（避免重复添加）
      const uniqueNewIds = newEmployeeIds.filter(
      ...
      if (searchValue) {
        ...
        if (node.type === 'employee') {
          ...
          employeeIdsToRemove = employeesInSearchResult.map((emp) =>
            Number(emp.id),
          )
        }
      } else {
      ...
      const candidates = selectedIds.filter(
        (id) => !employeeIdsToRemove.includes(id) || lockedIds.includes(id),
      )
  ...
  // 将禁用成员ID映射到树节点key格式
  const getTreeDisabledKeys = (): {
    id: string | number
    type: PersonnelType
  }[] => {
    const convertedDisabledIds = convertUinToEmployeeId(disabledIds)
    return convertedDisabledIds.map((id) => ({
      id,
      type: 'employee' as PersonnelType,
    }))
  }
  ...
  const convertEmployeeIdToUin = (
    ...
    employeeIds.forEach((employeeId) => {
      const emp = employees.find(
        (e) => Number((e as any).id) === Number(employeeId),
      )
  ...
  const handleConfirm = () => {
    ...
    if (failed.length > 0) {
      message.error(
        t('app.docs.detail.permissionUpdateFailed', {
          defaultValue: '存在无法匹配到当前公司账号的成员，请检查选择',
        }),
      )
      return
    }
      ...
      <div className='bg-white rounded-lg flex flex-col gap-2.5'>
        ...
        <div className='flex py-3 items-center justify-between border-b border-[#EFF1F4]'>
          <span className='text-sm font-medium text-[#000000]'>
            {t('app.docs.detail.selectMembers')}
          </span>
          <button
            onClick={onClose}
        ...
        <div className='flex flex-1 gap-4'>
          ...
          <div className='flex flex-col gap-5 w-[263px] bg-white'>
            ...
            <div
              className={`overflow-y-auto rounded-lg pr-2 ${scrollStyles.scroll}`}
              style={{ height: '430px' }}
            >
              <PersonnelTree
                key={treeKey}
                ...
                disabledIds={getTreeDisabledKeys()}
          ...
          <div className='w-[287px] p-[10px] rounded-md bg-[#FAFAFA] relative h-[480px] flex flex-col'>
            ...
            <div
              className={`overflow-y-auto rounded-lg ${scrollStyles.scroll} flex-1`}
            >
              {selectedMembers.length > 0 ? (
                      ...
                      >
                        <div className='flex items-center gap-2 flex-1'>
                          <img
                            ...
                            className='w-4 h-4 flex-shrink-0'
                          />
                          <span className='text-sm font-medium text-[#0C1F17]'>
                            {member.name}
                          </span>
                        </div>
                        <button
