...
export const ModifySegmentRuleModal: FC<ModifySegmentRuleModalProps> = ({
  ...
  const [selectAll, setSelectAll] = useState(() => {
    const preprocessing = initialRule?.textPreprocessing
    if (!preprocessing) return false
    return (
      preprocessing.removeExtraSpaces &&
      preprocessing.removeLineBreaks &&
      preprocessing.removeSpecialChars
    )
  })
  ...
  const handleRuleTypeChange = useCallback(
    (e: RadioChangeEvent) => {
      ...
      setRuleType(value)
      if (value === 'custom') {
        // 切换到自定义规则时，如果有初始值则使用初始值，否则使用默认值
        ...
        setTextPreprocessing(initialPreprocessing)
        const allSelected = Object.values(initialPreprocessing).every(Boolean)
  ...
  const hasChanges = useCallback(() => {
    // 如果类型不同，说明有更改
    if (ruleType !== initialRule?.type) {
      return true
    }

    // 如果选择的是默认规则，不需要检查其他字段
    if (ruleType === 'default') {
      return false
    }
    ...
    if (ruleType === 'custom') {
      return (
        segmentLength !== (initialRule?.segmentLength || 256) ||
        segmentSeparator !== (initialRule?.segmentSeparator || '\n') ||
        segmentOverlap !== (initialRule?.segmentOverlap || 30) ||
        textPreprocessing.removeExtraSpaces !==
          (initialRule?.textPreprocessing?.removeExtraSpaces ?? false) ||
        textPreprocessing.removeLineBreaks !==
          (initialRule?.textPreprocessing?.removeLineBreaks ?? false) ||
        textPreprocessing.removeSpecialChars !==
          (initialRule?.textPreprocessing?.removeSpecialChars ?? true)
      )
    }
  ...
  return (
    <Modal
      title={
        <span className='text-[18px] text-[#1E1F28] font-medium'>
          修改分段规则
        </span>
      }
      open={open}
            ...
            <div className='flex flex-col gap-2'>
              ...
              <div className='w-full'>
                <div className='relative w-full'>
                  ...
                  />
                  <span className='absolute right-[1px] top-[1px] transform w-[38px] h-[30px] rounded-r border-l border-0 border-[#D7D9E5] flex items-center justify-center bg-[#F7F8FA] text-sm text-[#86909C] pointer-events-none'>
                    字
                  </span>
                </div>
              </div>
