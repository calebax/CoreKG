...
export const NodeDetail: FC<
  Style & {
    ...
    clearItem: () => void
    onViewDetail?: (nodeName: string, tagName: string) => void
  }
> = (props) => {
  const { item, clearItem, className, style, onViewDetail } = props
      ...
      {!tags || tags.length === 0 ? (
        ...
        <div className='flex-1 flex flex-col overflow-hidden'>
          ...
          <Properties
            ...
            key={activeKey}
            value={tags
              .find((item) => item.name === activeKey)!
              .properties_values?.filter((item) =>
