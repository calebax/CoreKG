...
后续会同时承载 Project、KnowledgeBase、Folder、File、Task、Artifact、Assistant、Workflow 等对象。如果每类对象都单独设计成员表和权限表，权限体系会快速分裂，例如 `ProjectMember`、知识库授权、文件授权、任务授权各自维护一套逻辑。

目标方案采用统一资源权限模型：

- 所有可授权对象都是 `ke_resource`。
- 所有主体和资源的身份关系都是 `ke_resource_binding`。
...
```sql
CREATE TABLE IF NOT EXISTS ke_resource (
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_ke_resource_active_biz
    ON ke_resource (org_id, type, biz_id)
    WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_ke_resource_org_type
    ON ke_resource (org_id, type)
    WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_ke_resource_uin
    ON ke_resource (uin)
    WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_ke_resource_parent
    ON ke_resource (parent_resource_id)
    WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_ke_resource_parent_resource_path_ids
    ON ke_resource USING GIN (parent_resource_path_ids)
    WHERE deleted_at IS NULL;
```
...
- 软删除资源不得再作为权限判断对象。
- 业务表创建对象时必须同步创建对应 `ke_resource` 记录。
...
```sql
CREATE TABLE IF NOT EXISTS ke_resource_binding (
    id BIGSERIAL PRIMARY KEY,
    org_id INT8 NOT NULL,
    uin INT8 NOT NULL DEFAULT 0,
    ...
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMPTZ NULL,
    CONSTRAINT fk_ke_resource_binding_resource
        FOREIGN KEY (resource_id)
        REFERENCES ke_resource (id)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_ke_resource_binding_active_principal
    ON ke_resource_binding (resource_id, principal_type, principal_id)
    WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_ke_resource_binding_principal
    ON ke_resource_binding (org_id, principal_type, principal_id)
    WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_ke_resource_binding_uin
    ON ke_resource_binding (uin)
    WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_ke_resource_binding_resource
    ON ke_resource_binding (resource_id)
    WHERE deleted_at IS NULL;
```
...
- `ke_resource_binding.org_id` 必须与对应 `ke_resource.org_id` 一致。
...
4. 校验 ke_resource.org_id == caller.OrgID。
...
```text
projects.id = 1001
ke_resource(type='project', biz_id=1001, parent_resource_id=null, parent_resource_path_ids='{}')
ke_resource_binding(resource_id=project_resource.id, principal=user:creator, identity='owner')
```

创建文件：

```text
ke_resource(type='file', biz_id=file.id, parent_resource_id=project_resource.id, parent_resource_path_ids='{project_resource.id}')
```
...
```text
ke_resource(type='artifact', biz_id=artifact.id, parent_resource_id=project_resource.id, parent_resource_path_ids='{project_resource.id}')
```
...
ke_resource(type='task', biz_id=task.id, parent_resource_id=project_resource.id, parent_resource_path_ids='{project_resource.id}')
```
...
```sql
CREATE TABLE IF NOT EXISTS ke_artifact_sources (
...
- 老数据中的项目 owner/admin/member 迁移为项目资源上的 `ke_resource_binding`。
...
- 成员/协作者管理统一展示 `ke_resource_binding`，不再按 Project、File、Artifact 分裂。

## 7. 建设范围与验收

后端必须交付：

- `ke_resource` 表、DAO 和资源树维护。
- `ke_resource_binding` 表、DAO 和绑定管理。
